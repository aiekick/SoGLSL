...
Texture3DPtr Texture3D::createComputeVolume(std::string vFormat, ct::ivec3 vVolumeSize, bool vGenMipMap,
	std::string vWrap, std::string vFilter)
{
	auto res = std::make_shared<Texture3D>();
...
Texture3DPtr Texture3D::createFromBuffer(int vCountChannels, float *vBuffer, size_t vCount, ct::ivec3 vVolumeSize, bool vGenMipMap,
	std::string vWrap, std::string vFilter)
{
	auto res = std::make_shared<Texture3D>();
...
ctTexturePtr Texture3D::PrepareFromFile(const char* vFilePathName, std::string vType, bool vGenMipMap, std::string vWrap, std::string vFilter)
{
	...
	if (buffer && res)
	{
		...
		if (res->glformat != 0 && res->glinternalformat != 0 && res->gldatatype != 0 && res->w > 0 && res->h > 0 && res->d > 0)
		{
			...
			glTexImage3D(GL_TEXTURE_3D, 0, res->glinternalformat,
				(GLsizei)res->w, (GLsizei)res->h, (GLsizei)res->d,
				0, res->glformat, res->gldatatype, buffer);
...
ctTexturePtr Texture3D::PrepareFromBuffer(int vCountChannels, float *vBuffer, size_t vCount, ct::ivec3 vVolumeSize, bool vGenMipMap, std::string vWrap, std::string vFilter)
{
	...
	glTexImage3D(GL_TEXTURE_3D, 0, res->glinternalformat,
		(GLsizei)res->w, (GLsizei)res->h, (GLsizei)res->d,
		0, res->glformat, res->gldatatype, vBuffer);

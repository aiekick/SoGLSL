...
int MeshPlySaver(std::string vFilePathName,
    ...
    if (!vFilePathName.empty()) {
        ...
        if (countIndices > 0) {
            for (size_t i = 0; i < countVertexs; i++) {
                ...
                int64_t firstTimeMark = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
                ...
                if (vExportTexCoords) {
                    if (vExportNormals) {
                        if (vExportVertexColor) {
                            n = snprintf(vertBuffer,
                                         512,
                                         "%.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %i %i %i %i\n",
                                         vn->p.x,
                                         vn->p.y,
                                         vn->p.z,
                                         vn->n.x,
                            ...
                            n = snprintf(
                                vertBuffer, 256, "%.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n", vn->p.x, vn->p.y, vn->p.z, vn->n.x, vn->n.y, vn->n.z, vn->t.x, vn->t.y);
                        }
                    } else {
                        if (vExportVertexColor) {
                            n = snprintf(vertBuffer,
                                         512,
                                         "%.5f %.5f %.5f %.5f %.5f %i %i %i %i\n",
                                         vn->p.x,
                                         vn->p.y,
                                         vn->p.z,
                                         vn->t.x,
                ...
                int64_t secondTimeMark = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
            ...
            if (vWorking) {
                if (vExportFaces && countIndices > 0) {
                    for (size_t i = 0; i < countIndices; i += 3) {
                        ...
                        n = snprintf(FaceBuffer, 256, "3 %i %i %i\n", indices[i + 0 + 0], indices[i + 0 + 1], indices[i + 0 + 2]);
...
int MeshFgaSaver(std::string vFilePathName,
                 std::atomic<double>& vProgress,
                 std::atomic<bool>& vWorking,
                 std::atomic<double>& vGenerationTime,
    ...
    if (!vFilePathName.empty()) {
        ...
        if (!count.emptyAND()) {
            ...
            if (countVectorToExport <= countVertexs) {
                ...
                n = snprintf(lineBuffer, 1023, ",%.5f,%.5f,%.5f\n", minBoundingBox.x, minBoundingBox.y, minBoundingBox.z);
                vectorStr += lineBuffer;
                n = snprintf(lineBuffer, 1023, ",%.5f,%.5f,%.5f\n", maxBoundingBox.x, maxBoundingBox.y, maxBoundingBox.z);
                vectorStr += lineBuffer;
...
inline void InfosPane(std::string vFilter, IGFDUserDatas /*vUserDatas*/, bool* vCantContinue) {
    if (vFilter == ".ply")  // ply
    {
        ...
        if (ImGui::Checkbox("Export Normals", &val)) {
            MeshSaver::exportNormals = val;
        }
        val = MeshSaver::exportTexCoords;
        if (ImGui::Checkbox("Export Tex Coords", &val)) {
            MeshSaver::exportTexCoords = val;
        }
        val = MeshSaver::exportVertexColor;
        if (ImGui::Checkbox("Export Vertex Color", &val)) {
            MeshSaver::exportVertexColor = val;
        }
        val = MeshSaver::exportFaces;
        if (ImGui::Checkbox("Export Faces", &val)) {
            MeshSaver::exportFaces = val;
        }
        if (vCantContinue)
            *vCantContinue = true;
    } else if (vFilter == ".fga")  // fga
    {
...

        {
            ...
            if (ImGui::InputInt("Count X", &val)) {
                MeshSaver::countVectorX = val;
            }
            val = MeshSaver::countVectorY;
            if (ImGui::InputInt("Count Y", &val)) {
                MeshSaver::countVectorY = val;
            }
            val = MeshSaver::countVectorZ;
            if (ImGui::InputInt("Count Z", &val)) {
                MeshSaver::countVectorZ = val;
            }
            ImGui::Unindent();

            if (MeshSaver::countVectorX <= 0 || MeshSaver::countVectorY <= 0 || MeshSaver::countVectorZ <= 0) {
...

        {
            ...
            if (ImGui::InputFloat("min X", &val)) {
                MeshSaver::minBoundX = val;
            }
            val = MeshSaver::minBoundY;
            if (ImGui::InputFloat("min Y", &val)) {
                MeshSaver::minBoundY = val;
            }
            val = MeshSaver::minBoundZ;
            if (ImGui::InputFloat("min Z", &val)) {
                MeshSaver::minBoundZ = val;
            }
            ImGui::Unindent();
            ...
            if (ImGui::InputFloat("max X", &val)) {
                MeshSaver::maxBoundX = val;
            }
            val = MeshSaver::maxBoundY;
            if (ImGui::InputFloat("max Y", &val)) {
                MeshSaver::maxBoundY = val;
            }
            val = MeshSaver::maxBoundZ;
            if (ImGui::InputFloat("max Z", &val)) {
                MeshSaver::maxBoundZ = val;
            }
            ImGui::Unindent();

            if (MeshSaver::maxBoundX - MeshSaver::minBoundX <= 0.0f || MeshSaver::maxBoundY - MeshSaver::minBoundY <= 0.0f ||
...
void MeshSaver::OpenDialog() {
    if (puFilePath.empty()) {
        puFilePath = FileHelper::Instance()->GetRegisteredPath((int)FILE_LOCATION_Enum::FILE_LOCATION_EXPORT);
    }
...
void MeshSaver::ShowDialog(ct::ivec2 vScreenSize) {
    ...
    if (ImGuiFileDialog::Instance()->Display("SaveMeshFileDialog", ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoDocking, min, max)) {
...
void MeshSaver::CreateThread(/*std::function<void()> vFinishFunc, */ MeshFormatEnum vMeshFormat) {
    if (!StopWorkerThread()) {
        if (vMeshFormat == MeshFormatEnum::MESH_FORMAT_PLY) {
            ...
            puWorkerThread = std::thread(MeshPlySaver,
                                         fileToLoad,
            ...
            // puFinishFunc = vFinishFunc;
            MeshSaver::Working = true;
            puWorkerThread = std::thread(MeshFgaSaver,
                                         fileToLoad,
                                         std::ref(MeshSaver::Progress),
                                         std::ref(MeshSaver::Working),
                                         std::ref(MeshSaver::GenerationTime),

...
inline static void sLoadMesh(std::atomic<double>& vProgress, std::atomic<bool>& vWorking, std::atomic<double>& vGenerationTime) {
    vProgress = 0.0;
    ...
    if (!filePathName.empty() && meshPtr) {
        ...
        try {
            ...
            const int64_t secondTimeMark = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
void MeshLoader::OpenDialog(RenderPackWeak vRenderpack) {
    ...
    if (puFilePath.empty()) {
        puFilePath = FileHelper::Instance()->GetRegisteredPath((int)FILE_LOCATION_Enum::FILE_LOCATION_ASSET_MESH);
    }
    ...
    ImGuiFileDialog::Instance()->OpenDialog("OpenMeshFileDialog", "Open Mesh File", "3D Files {.obj,.ply,.gltf,.fbx,.stl,.3ds,.dae,.lwo,.blend}", config);
}
...
void MeshLoader::ShowDialog(ct::ivec2 vScreenSize) {
    ...
    if (ImGuiFileDialog::Instance()->Display("OpenMeshFileDialog", ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoDocking, min, max)) {
...
void MeshLoader::CreateThread(std::function<void()> vFinishFunc) {
    if (!StopWorkerThread()) {
        ...
        if (modelPtr) {
            ...
            puWorkerThread = std::thread(sLoadMesh,
                                         // puMeshLoaderThread,
                                         // modelPtr,

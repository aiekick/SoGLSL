...
PNTCModel::PNTCModel(const GuiBackend_Window& vWin)
	: BaseModel(vWin)
{
...
void PNTCModel::DrawModel(const std::string& vName, const GLenum& vRenderMode, const bool& vUseTesselation)
{
	...
	if (glIsVertexArray(m_MeshDatas.m_Vao) == GL_TRUE)
	{
		if (m_IndicesCountToShow > 0 && m_IndicesCountToShow <= m_IndicesCount)
		{
			...
			{
				glDrawElementsInstanced(vRenderMode, (GLsizei)m_IndicesCountToShow, GL_UNSIGNED_INT, nullptr, (GLsizei)m_InstanceCount);	// draw first object 6 => decalage 3 coord * 2 (float)
				LogGlError();
			}
			else
			{

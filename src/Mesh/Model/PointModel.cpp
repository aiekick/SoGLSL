...
PointModel::PointModel(const GuiBackend_Window& vWin, const uint64_t& vVerticesCount)
	: BaseModel(vWin)
{
...
bool PointModel::PreparePoint(uint64_t vVerticesCount, bool vUpdate)
{
	...
	if (!vUpdate)
	{
		if (m_MeshDatas.m_Vbo > 0) { glDeleteBuffers(1, &m_MeshDatas.m_Vao); }
		if (m_MeshDatas.m_Vao > 0) { glDeleteVertexArrays(1, &m_MeshDatas.m_Vbo); }
	...
	glBufferData(GL_ARRAY_BUFFER, 
		...
		m_MeshDatas.m_Vertices.data(), GL_STATIC_DRAW);

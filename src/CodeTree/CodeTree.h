...
public:
	...
	ShaderKeyPtr AddOrUpdateFromStringAndGetKey(
		const std::string& vKey, 
		const std::string& vFileString, 
		const std::string& vOriginalFilePathName, // like the oriignal file from where the current code (vFileString) come from
		const std::string& vInFileBufferName, // like MAIN or CHILD_A, CHILD_B
	...
	bool DrawImGuiUniformWidget(ShaderKeyPtr vKey, float vFirstColumnWidth, RenderPackWeak vRenderPack = RenderPackWeak(), bool vShowUnUsed = true, bool vShowCustom = false);
	bool DrawImGuiIncludesUniformWidget(RenderPackWeak vMainRenderPack, float vFirstColumnWidth, ct::ivec2 vScreenSize);
	bool DrawImGuiIncludesUniformWidget(UniformsMultiLoc *vUniLoc, float vFirstColumnWidth, RenderPackWeak vMainRenderPack = RenderPackWeak(), bool vShowUnUsed = true, bool vShowCustom = false);
	bool DrawImGuiUniformWidgetForPanes(UniformVariantPtr vUniPtr, float vMaxWidth, float vFirstColumnWidth, RenderPackWeak vRenderPack = RenderPackWeak(), bool vShowUnUsed = true, bool vShowCustom = false);
	...
	//void RemoveKeyToReplaceInShadertype(const std::string& vKey, const std::string& vKeyInCode, const std::string& vShaderType);

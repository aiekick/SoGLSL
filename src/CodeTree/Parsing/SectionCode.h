...
public:
    ...
    static std::shared_ptr<SectionCode> Create(ShaderKeyPtr vParentkey,
...
public:
    SectionCode();
    SectionCode(std::weak_ptr<ShaderKey> vParentKey,
                std::shared_ptr<SectionCode> vParentSection,
    ...
    std::string GetSectionPart(const std::string& vSectionType,
...
private:
    ...
    void DefineSubSection(const std::string& vNewCode, size_t vStartPos, size_t vEndPos, std::string vType, std::string vParentType, bool vIsInclude);
    std::string GetShaderPart(std::shared_ptr<SectionCode> vSectionCodeParent,

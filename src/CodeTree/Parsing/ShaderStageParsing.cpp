...
void SceneConfigStruct::Complete(SceneConfigStruct* vConfigToComplete)
{
	if (vConfigToComplete && countChanges)
	{
		...
		if (shaders != _default.shaders)	vConfigToComplete->shaders = shaders;
	}
}
...
void FrameBufferShaderConfigStruct::Complete(FrameBufferShaderConfigStruct* vConfigToComplete)
{
	if (vConfigToComplete)
	{
		if (countChanges)
		{
			...
			if (countIterations != _default.countIterations)	vConfigToComplete->countIterations = countIterations;
			if (ratio > _default.ratio)							vConfigToComplete->ratio = ratio;
			if (size != _default.size)							vConfigToComplete->size = size;
			if (format != _default.format)						vConfigToComplete->format = format;
			if (mipmap != _default.mipmap)						vConfigToComplete->mipmap = true;
			if (wrap != _default.wrap)							vConfigToComplete->wrap = wrap;
			if (filter != _default.filter)						vConfigToComplete->filter = filter;
			if (count != _default.count)						vConfigToComplete->count = count;
			if (bufferName != _default.bufferName)				vConfigToComplete->bufferName = bufferName;
		}
	}
}
...
void FrameBufferShaderConfigStruct::CheckChangeWith(FrameBufferShaderConfigStruct* vConfigToCompareForChanges)
{
	if (vConfigToCompareForChanges)
	{
		...
		if (IS_FLOAT_DIFFERENT(ratio, vConfigToCompareForChanges->ratio))		needSizeUpdate |= true;
		if (size.x != vConfigToCompareForChanges->size.x)						needSizeUpdate |= true;
		if (size.y != vConfigToCompareForChanges->size.y)						needSizeUpdate |= true;

		needFBOUpdate = false;

		if (countIterations != vConfigToCompareForChanges->countIterations)		needCountIterationsUpdate |= true;
		if (format != vConfigToCompareForChanges->format)						needFBOUpdate |= true;
		if (mipmap != vConfigToCompareForChanges->mipmap)						needFBOUpdate |= true;
		if (wrap != vConfigToCompareForChanges->wrap)							needFBOUpdate |= true;
		if (filter != vConfigToCompareForChanges->filter)						needFBOUpdate |= true;
		if (count != vConfigToCompareForChanges->count)							needFBOUpdate |= true;
	}
}
...
{
	// apply default before check
	if (meshType == BaseMeshEnum::PRIMITIVE_TYPE_NONE)	meshType = BaseMeshEnum::PRIMITIVE_TYPE_QUAD;
	if (defaultDisplayMode.empty())					defaultDisplayMode = "TRIANGLES";
	if (displayMode.empty())						displayMode[defaultDisplayMode] = ModelRenderModeEnum::MODEL_RENDER_MODE_TRIANGLES;
}
...
void VertexShaderConfigStruct::Complete(VertexShaderConfigStruct* vConfigToComplete)
{
	if (vConfigToComplete && countChanges)
	{
		...
		if (type != _default.type)									vConfigToComplete->type = type;
		if (countInstances != _default.countInstances)				vConfigToComplete->countInstances = countInstances;
		if (lineWidth != _default.lineWidth)						vConfigToComplete->lineWidth = lineWidth;
		if (countVertexs != _default.countVertexs)					vConfigToComplete->countVertexs = countVertexs;
		if (meshType != _default.meshType)							vConfigToComplete->meshType = meshType;
		if (modelFileToLoad != _default.modelFileToLoad)			vConfigToComplete->modelFileToLoad = modelFileToLoad;
		if (defaultDisplayMode != _default.defaultDisplayMode)		vConfigToComplete->defaultDisplayMode = defaultDisplayMode;
		if (displayMode != _default.displayMode)					vConfigToComplete->displayMode = displayMode;
	}
}
...
void VertexShaderConfigStruct::CheckChangeWith(VertexShaderConfigStruct* vConfigToCompareForChanges)
{
	if (vConfigToCompareForChanges)
	{
		...
		if (lineWidth != vConfigToCompareForChanges->lineWidth)											needLineWidthUpdate |= true;
		if (countInstances != vConfigToCompareForChanges->countInstances)									needCountPointUpdate |= true;
		if (countVertexs != vConfigToCompareForChanges->countVertexs)										needCountPointUpdate |= true;
		if (meshType != vConfigToCompareForChanges->meshType)												needModelUpdate |= true;
		if (modelFileToLoad != vConfigToCompareForChanges->modelFileToLoad)								needModelUpdate |= true;
		if (defaultDisplayMode != vConfigToCompareForChanges->defaultDisplayMode)							needDisplayModeUpdate |= true;
		if (displayMode.size() != vConfigToCompareForChanges->displayMode.size())							needDisplayModeUpdate |= true;
...
void ComputeShaderConfigStruct::Complete(ComputeShaderConfigStruct* vConfigToComplete)
{
	if (vConfigToComplete && countChanges)
	{
		...
		if (size != _default.size)							vConfigToComplete->size = size;
		if (workgroups != _default.workgroups)				vConfigToComplete->workgroups = workgroups;
		if (countIterations != _default.countIterations)	vConfigToComplete->countIterations = countIterations;
	}
}
...
void ComputeShaderConfigStruct::CheckChangeWith(ComputeShaderConfigStruct* vConfigToCcompareForChanges)
{
	if (vConfigToCcompareForChanges)
	{
		...
		if (size != vConfigToCcompareForChanges->size) needSizeUpdate |= true;
		if (workgroups != vConfigToCcompareForChanges->workgroups) needWorkgroupUpdate |= true;
		if (countIterations != vConfigToCcompareForChanges->countIterations) needCountIterationsUpdate |= true;
	}
}
		...
		if (tagPos != std::string::npos)
		{
			...
			else
			{
				...
				if (endtag != std::string::npos)
				{
					...
					if (tagFoundToCompare == SectionCode::codeTags.ConfigStartTag ||
						tagFoundToCompare == SectionCode::codeTags.ConfigEndTag ||
						tagFoundToCompare == SectionCode::codeTags.BufferStartTag ||
						tagFoundToCompare == SectionCode::codeTags.BufferEndTag)
					{
						tagFound.clear();
					}
				}
...
void ShaderParsedStruct::ParseNote(const std::string& vNote)
{
	...
	while (lastFound != std::string::npos)
	{
		...
		if (namePos == std::string::npos &&
			datePos == std::string::npos &&
			urlPos == std::string::npos &&
			parentUrlPos == std::string::npos &&
			userPos == std::string::npos &&
			soundUrlPos == std::string::npos)
		{
...
bool ShaderStageParsing::ParseSectionConfig_ReturnSectionName_SayIfWeCanUse(
...
{
	if (!vSectionCode) return false;
	if (vSectionCode->parentKey.expired()) return false;
	if (!vSectionNameToReturn) return false;
	if (!vInFileBufferNameToReturn) return false;
...
void ShaderStageParsing::ParseNoteConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vTags);
	UNUSED(vCurrentFileLine);
...
ProjectConfigStruct ShaderStageParsing::ParseProjectConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vCurrentFileLine);
...
SceneConfigStruct ShaderStageParsing::ParseSceneConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vCurrentFileLine);
...
FrameBufferShaderConfigStruct ShaderStageParsing::ParseFrameBufferConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	FrameBufferShaderConfigStruct config;

	if (!vSectionCode) return config;
		...
		if (conf.params.size() == 1) // 1 seul tag
		{
			...
			if (conf.tag == "SIZE")
			{
				...
				if (params.size() == 1)
				{
					...
					{
						ct::ivec3 _size(
							ct::ivariant(params[0][0]).GetI(),
							ct::ivariant(params[0][1]).GetI(),
							0);
							...
							else
							{
								...
								//if (pKey)
								{
									if (n < 1)
									{
										...
										vSectionCode->SetSyntaxError(vSectionCode->relativeFile, "FBO Config error", false, 
											"the FBO count must be comprised between 1 and 8", vCurrentFileLine);
									...
									else if (n > 8)
									{
										...
										vSectionCode->SetSyntaxError(vSectionCode->relativeFile, "FBO Config error", false, 
											"A Framebuffer can't have more than 8 attachments\n FBO count must be comprised between 1 and 8", vCurrentFileLine);
...
VertexShaderConfigStruct ShaderStageParsing::ParseVertexConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
				...
				if (!params.empty())
				{
					...
					for (const auto& itMode : params[0])
					{
						...
						else
						{
							...
							vSectionCode->SetSyntaxError(vSectionCode->relativeFile, "Section Display Type error", false, itMode + " is not a valid display mode", vCurrentFileLine);
						}
					}
					...
					if (params.size() > 1)
					{
						if (params[1].size() == 1)
						{
							...
							else
							{
								...
								vSectionCode->SetSyntaxError(vSectionCode->relativeFile, "Section Display Type error", false, confDefault + " is not in the display list", vCurrentFileLine);
			...
			else if (conf.tag == "LINEWIDTH")
			{
				...
				if (!params.empty())
				{
					...
					else if (params.size() == 3)
					{
						...
						if (params[0].size() == 1U) inf = ct::fvariant(params[0][0]).GetF();
						if (params[1].size() == 1U) sup = ct::fvariant(params[1][0]).GetF();
						if (params[2].size() == 1U) def = ct::fvariant(params[2][0]).GetF();
	...
	if (!haveDisplayTag)
	{
		...
		//vSectionCode->parentKey->vertexConfig.displayMode[GetModelRenderModeEnumString(ModelRenderModeEnum::vk::PrimitiveTopology::ePatchList)] = ModelRenderModeEnum::vk::PrimitiveTopology::ePatchList;
		//vSectionCode->parentKey->vertexConfig.displayMode[GetModelRenderModeEnumString(ModelRenderModeEnum::vk::PrimitiveTopology::eLineStrip_ADJACENCY)] = ModelRenderModeEnum::vk::PrimitiveTopology::eLineStrip_ADJACENCY;
		//vSectionCode->parentKey->vertexConfig.displayMode[GetModelRenderModeEnumString(ModelRenderModeEnum::vk::PrimitiveTopology::eLineList_ADJACENCY)] = ModelRenderModeEnum::vk::PrimitiveTopology::eLineList_ADJACENCY;
		//vSectionCode->parentKey->vertexConfig.displayMode[GetModelRenderModeEnumString(ModelRenderModeEnum::vk::PrimitiveTopology::eTriangleStrip_ADJACENCY)] = ModelRenderModeEnum::vk::PrimitiveTopology::eTriangleStrip_ADJACENCY;
		//vSectionCode->parentKey->vertexConfig.displayMode[GetModelRenderModeEnumString(ModelRenderModeEnum::vk::PrimitiveTopology::eTriangleList_ADJACENCY)] = ModelRenderModeEnum::vk::PrimitiveTopology::eTriangleList_ADJACENCY;
	}

	return config;
}

ComputeShaderConfigStruct ShaderStageParsing::ParseComputeConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
			...
			if (conf.tag == "SIZE")
			{
				if (params.size() == 1) // SIZE(10,10,10)
				{
					...
					else
					{
						...
						vSectionCode->SetSyntaxError(vSectionCode->relativeFile, "Section Display Type error", false, "you must give 3 coords for size", vCurrentFileLine);
					}
				}
			}
...
void ShaderStageParsing::ParseGeometryConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vTags);
	UNUSED(vCurrentFileLine);
...
void ShaderStageParsing::ParseTesselationControlConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vTags);
	UNUSED(vCurrentFileLine);
...
void ShaderStageParsing::ParseTesselationEvalConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vTags);
	UNUSED(vCurrentFileLine);
...
void ShaderStageParsing::ParseCommonConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vTags);
	UNUSED(vCurrentFileLine);
...
void ShaderStageParsing::ParseFragmentConfig(std::shared_ptr<SectionCode> vSectionCode, const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags, size_t vCurrentFileLine)
{
	UNUSED(vSectionCode);
	UNUSED(vTags);
	UNUSED(vCurrentFileLine);
			...
			if (_extractCode.find('.', 0) == std::string::npos)
			{
				if (pkeyPtr->puParentCodeTree)
				{
					...
					if (key)
					{
						...
						{
							vSectionCode->SetSyntaxError(key, "Include Error", true, "Infinite Loop detected on Inclusion of " + key->puKey + ", script ignored !", vCurrentFileLine);
...
std::vector<GeometryLayoutStruct> ShaderStageParsing::SearchForGeometryLayout(
	const std::string& vCodeToParse, const std::string& vTagToSearch,

...
enum class BaseMeshEnum : uint8_t { PRIMITIVE_TYPE_QUAD = 0, PRIMITIVE_TYPE_POINTS, PRIMITIVE_TYPE_MESH, PRIMITIVE_TYPE_NONE, PRIMITIVE_TYPE_Count };
...
static inline BaseMeshEnum GetModelTypeEnumFromString(const std::string& vString) {
    if (vString == "QUAD")
        return BaseMeshEnum::PRIMITIVE_TYPE_QUAD;
    if (vString == "POINTS")
        return BaseMeshEnum::PRIMITIVE_TYPE_POINTS;
    if (vString == "MESH")
        return BaseMeshEnum::PRIMITIVE_TYPE_MESH;
...
public:
    ...
    static bool IsTagFound(const std::string& vCode,
                           const std::string& vTag,
                           size_t vStartPos,
                           size_t* vApproxPos,
                           bool vCheckIfInCommentZone = true,
                           bool vWordOnly = false);
...
public:
    ...
    bool ParseSectionConfig_ReturnSectionName_SayIfWeCanUse(std::shared_ptr<SectionCode> vSectionCode,
...
private:
    ...
    ProjectConfigStruct ParseProjectConfig(std::shared_ptr<SectionCode> vSectionCode,
                                           const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags,
                                           size_t vCurrentFileLine);
    SceneConfigStruct ParseSceneConfig(std::shared_ptr<SectionCode> vSectionCode,
                                       const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags,
                                       size_t vCurrentFileLine);
    FrameBufferShaderConfigStruct ParseFrameBufferConfig(std::shared_ptr<SectionCode> vSectionCode,
                                                         const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags,
                                                         size_t vCurrentFileLine);
    VertexShaderConfigStruct ParseVertexConfig(std::shared_ptr<SectionCode> vSectionCode,
                                               const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags,
                                               size_t vCurrentFileLine);
    ComputeShaderConfigStruct ParseComputeConfig(std::shared_ptr<SectionCode> vSectionCode,
                                                 const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags,
                                                 size_t vCurrentFileLine);
    ...
    void ParseTesselationControlConfig(std::shared_ptr<SectionCode> vSectionCode,
                                       const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags,
                                       size_t vCurrentFileLine);
    void ParseTesselationEvalConfig(std::shared_ptr<SectionCode> vSectionCode,
                                    const std::unordered_map<std::string, ConfigTagParsedStruct>& vTags,
                                    size_t vCurrentFileLine);
    ...
    std::vector<GeometryLayoutStruct> SearchForGeometryLayout(const std::string& vCodeToParse,

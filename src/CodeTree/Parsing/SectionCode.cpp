...
{
	std::shared_ptr<SectionCode> res = std::make_shared<SectionCode>(
		vParentkey, vParentSection, vFile, vCode, vType, vParentType,
		vName, vInFileBufferName, vSourceCodeStartLine, vSourceCodeEndLine, vLevel, vIsInclude);
	res->m_This = res;
	return res;
}
...
SectionCode::SectionCode(
	std::weak_ptr<ShaderKey> vParentKey, std::shared_ptr<SectionCode> vParentSection,
	...
	if (pkeyPtr.use_count())
	{
				...
				if (incKeyPtr.use_count())
				{
					...
					if (ps.isOk)
					{
						FileHelper::Instance()->SaveToFile(
							_extractCode, 
							"include_" + ps.name + ".txt", 
							(int)FILE_LOCATION_Enum::FILE_LOCATION_DEBUG);
					}
#endif
				}
				else
		...
		else if (vType == "CONFIG")
		{
			...
			if (_endLine != std::string::npos)
			{
				...
				{
					if (vParentType == "VERTEX" ||
						vParentType == "GEOMETRY" ||
						vParentType == "FRAGMENT" ||
						vParentType == "TESSCONTROL" ||
						vParentType == "TESSEVAL" ||
						vParentType == "COMPUTE")
					{
						pkeyPtr->AddConfigName(vParentType, it, name, _sectionName, true);
					}
					else if (vParentType == "UNIFORMS" ||
						vParentType == "FRAMEBUFFER")
					{
			...
			vType == "COMPUTE")*/
		{
			...
			if (_endLine != std::string::npos)
			{
				...
				bool useable = m_ShaderStageParsing.ParseSectionConfig_ReturnSectionName_SayIfWeCanUse(
					m_This, _configLine, vType, sourceCodeStartLine + sl, forcedStageName, &_sectionName, &_inFileBufferNames);
				_endLine += 1; ++sl;
				if (_endLine < _extractCode.size())	_extractCode = _extractCode.substr(_endLine);
				else _extractCode.clear();
				if (!useable) _extractCode.clear();
				++sl;
			}

			if (vType == "COMMON") pkeyPtr->puIsCommonShaderPartPresent = true;
			if (vType == "VERTEX") pkeyPtr->puIsVertexShaderPresent = true;
			if (vType == "GEOMETRY") pkeyPtr->puIsGeometryShaderPresent = true;
			if (vType == "TESSCONTROL") pkeyPtr->puIsTesselationControlShaderPresent = true;
			if (vType == "TESSEVAL") pkeyPtr->puIsTesselationEvalShaderPresent = true;
			if (vType == "FRAGMENT") pkeyPtr->puIsFragmentShaderPresent = true;
			if (vType == "COMPUTE") pkeyPtr->puIsComputeShaderPresent = true;
		}

		if (!_extractCode.empty() ||
			needToCreateThisSectionEvenIfCodeEmpty)
		{
			subSections[vStartPos].clear();
			if (_inFileBufferNames.empty())
			{
				if (!inFileBufferName.empty()) _inFileBufferNames.push_back(inFileBufferName);
				else _inFileBufferNames.emplace_back("");
			}
			for (auto& it : _inFileBufferNames)
			{
				auto _section = SectionCode::Create(
					pkeyPtr,
					m_This,
					_absoluteFile,
					_extractCode,
	...
	while (nextPos != std::string::npos)
	{
		...
		if (nextPos != std::string::npos)
		{
			if (!ShaderStageParsing::IfInCommentZone(codeToParse, nextPos))
			{
				...
				{
					DefineSubSection(codeToParse, vLastBlockPos, posWithoutTag,
						"TEXT",
						this->currentType, isInclude);
				}
				...
				if (nextPos != std::string::npos)
				{
					if (nextOpenTag != std::string::npos)
					{
						...
						{
							SetSyntaxError(relativeFile, "Config Syntax Error", true, "you must close a config flag, before open a new one",
					...
					if (!ShaderStageParsing::IfInCommentZone(codeToParse, nextPos))
					{
						if (pos)
						{
							...
							DefineSubSection(codeToParse, pos, nextPos,
								"CONFIG",
								this->currentType, isInclude);
			...
			if (!ShaderStageParsing::IfInCommentZone(codeToParse, nextPos))
			{
				...
				if (firstQuote != std::string::npos)
				{
					...
					if (endQuote != std::string::npos)
					{
						...
						{
							SetSyntaxError(relativeFile, "Include Syntax Error", true, "you must quote the include file on the same line.",
						...
						else
						{
							...
							if (endQuote != std::string::npos)
							{
								...
								DefineSubSection(codeToParse, firstQuote, endQuote,
									"INCLUDE",
				...
				{
					SetSyntaxError(relativeFile, "Include Syntax Error", true, "you must quote the include file.",
						ct::GetCountOccurenceInSection(codeToParse, 0, pos, '\n') + this->sourceCodeStartLine);
				}
			}
				...
				if (!ShaderStageParsing::IfInCommentZone(codeToParse, nextPos))
				{
					...
					if (coma_pos != std::string::npos)
					{
						...
						if (pos_layout != std::string::npos && pos_binding != std::string::npos)
						{
							...
							if (nextPos > pos_binding &&
								pos_binding > pos_layout &&
								pos_layout >= last_coma_pos)
							{
								uniformFound = false;
							}
						}
						...
						if (uniformFound)
						{
							...
							if (first_endline_pos != std::string::npos)
							{
								...
								if (el > sl) // on fait une section text que si elle fait au moin une ligne
								{
									DefineSubSection(codeToParse, vLastBlockPos, nextPos,
										"TEXT",
										this->currentType, isInclude);
								}

								DefineSubSection(codeToParse, nextPos, first_endline_pos,
									"UNIFORM",
									this->currentType, isInclude);
			...
			if (!ShaderStageParsing::IfInCommentZone(codeToParse, nextPos) && stageName != "NOTE")
			{
				...
				if (nextPos != std::string::npos)
				{
					if (nextOpenTag != std::string::npos)
					{
						if (nextOpenTag < nextPos)
						{
							SetSyntaxError(relativeFile, "Replace Code Syntax Error", true, "you must close a Replace flag, before open a new one",
								ct::GetCountOccurenceInSection(codeToParse, 0, nextOpenTag, '\n') + this->sourceCodeStartLine);
						}
					}
					if (!ShaderStageParsing::IfInCommentZone(codeToParse, nextPos))
...
bool SectionCode::ParseFragColorLayouts(const std::string& codeToParse)
{
	...
	*/
	...
	{
		if (parentKey.expired()) return false;
		auto pkeyPtr = parentKey.lock();
		if (pkeyPtr.use_count())
		{
			size_t nextPos = 0;
	...
	if (!code.empty())
	{
			...
			for (auto& section : sections)
			{
				...
				if (idx > 0)
				{
					if (!last_tag.empty())
					{
						...
						DefineSubSection(codeToParse, last_idx + last_tag.size() + 1, idx,
							last_tag,
							parentType, isInclude);
					}
				}
		...
		else
		{
			...
			if (lastBlockPos > 0)
			{
				...
				{
					DefineSubSection(codeToParse, lastBlockPos, lastSectionPos,
						"TEXT", this->currentType, isInclude);
				}
			}
		}
...
std::string SectionCode::GetShaderPart(
	...
	bool vFound, int vLevel)
{
	...
	{	
		...
		if (currentType == "COMMON")
		{
			if (vRootType == "VERTEX" ||
				vRootType == "GEOMETRY" ||
				vRootType == "FRAGMENT" ||
				vRootType == "TESSCONTROL" ||
				vRootType == "TESSEVAL")
				vFound = true;
		}
		else if (!_desiredStageName.empty())
		{
			if (vRootType != "UNIFORMS" &&
				vRootType != "COMMON")
			{
	...
	{
		if (!name.empty() &&
			!vDesiredConfigName.empty() &&
			name != vDesiredConfigName)
		{
	...
	if (!subSections.empty())
	{
		...
		for (auto& subSection : subSections)
		{
			for (auto sec : subSection.second)
			{
				...
				std::string _code = sec->GetShaderPart(
					m_This,
					_sectionType,
...
std::string SectionCode::GetSectionPart(
	const std::string& vSectionType,
	size_t vCurrentFinalLine,
	...
	std::map<KEY_TYPE_Enum, std::set<std::string>>* vUsedFiles)
{
	...
	if (!vSectionType.empty() && !subSections.empty())
	{
		...
		for (auto& subSection : subSections)
		{
			for (auto sec : subSection.second)
			{
				if (sec->currentType == vOriginType ||
					sec->currentType == vParentType ||
					sec->currentType == "COMMON")
				{
					std::string _code = sec->GetShaderPart(
						m_This,
						vSectionType,
...
void SectionCode::SetSyntaxError(std::weak_ptr<ShaderKey>  vKey, const std::string & vErrorType, bool vErrorOrWarnings, const std::string & vError, size_t vLine)
{
	...
	if (vKeyPtr.use_count())
	{
		...
		vKeyPtr->GetSyntaxErrors()->SetSyntaxError(vKeyPtr, "ERROR", vErrorType, vErrorOrWarnings,
			LineFileErrors(vLine, fromFile, vError));
	}
}

...
enum class CONFIG_TYPE_Enum
{
	CONFIG_TYPE_ALL = 0,
	CONFIG_TYPE_SHADER = 1,
	CONFIG_TYPE_UNIFORM = 2
};
...
public:
	...
	std::map<std::string, // include name
		std::string> puIncludeFileNames; // corresponding valid path
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	std::map<std::string,  // in file buffer name
		...
		UniformParsedStruct>>>>> puUniformParsedDataBase; // uniform
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	std::map<std::string,  // shader stage name
		std::map<std::string,  // uniform name
		FinalUniformStruct>> puFinalUniformsCode;	// contient les formes final des uniforms
...
public:
	SyntaxErrors* GetSyntaxErrors() { return &puSyntaxErrors; }
...
public:
	...
	// configNames
	void ClearConfigs();
	std::set<std::string>* GetConfigNames(
		const std::string& vShaderStageName,
		const std::string& vInFileBufferName,
		const std::string& vSectionName);
	void AddConfigName(
	...
	void CreateUniformsHeader(
		const std::string& vInFileBufferName,
		const std::string& vStageName,
		const std::string& vSectionName,
		const std::string& vConfigName);
	void CreateUniformsHeaderFromParsedStruct(const std::string& vSectionType, const UniformParsedStruct& vUniformParsedStruct, std::map<std::string, std::string> *vTempDico);
	...
	bool SetNewCodeInReplaceCodes(const std::string& vShaderCodePartEnum, const std::string& vName, const std::string& vCodeToReplace, std::string vNewCodeFromKey, bool vOnlyIfNotAlreadyDefined = false);
	void ClearReplaceCodes();

	// config files *.conf
	void LoadConfigShaderFile(
		const std::string& vShaderFileName,
		const CONFIG_TYPE_Enum& vConfigType,
		const std::string& vUniformConfigName);
	void SaveConfigShaderFile(std::string vShaderFileName, CONFIG_TYPE_Enum vConfigType, std::string vUniformConfigName);

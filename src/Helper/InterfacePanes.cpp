...
void InterfacePanes::DisplayMessageOfRenderPack(const GuiBackend_Window& vWin, const bool& vHideWarnings, CodeTreePtr vCodeTree, 
...
void InterfacePanes::DisplayScriptCode(const GuiBackend_Window& vWin, CodeTreePtr  /*vCodeTree*/, RenderPackWeak vRenderPack, const bool& vUseTextEditor)
{
	...
	if (rpPtr && rpPtr->GetShaderKey())
	{
		...
		{
			if (ImGui::RadioButtonLabeled(ImVec2(0.0f,0.0f), "Vertex", "Show/Hide Vertex", puShowCodeSection == 0)) { puShowCodeSection = 0; puForceRefreshCode = true; }
...
void InterfacePanes::DisplayCodeTreeSectionRecurs(std::shared_ptr<SectionCode> vSectionCode, std::string vSectionNameForErrorsLines)
{
	if (vSectionCode)
	{
			...
			for (auto it = vSectionCode->subSections.begin(); it != vSectionCode->subSections.end(); ++it)
			{
				for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2)
				{
					...
					if (ImGui::TableNextColumn())
						opened = ImGui::TreeNodeEx("##sectionTreeNode", ImGuiTreeNodeFlags_SpanFullWidth | ImGuiTreeNodeFlags_SpanAvailWidth, "%s",	sec->currentType.c_str());
					if (ImGui::TableNextColumn()) ImGui::Text("%s", sec->parentType.c_str());
					if (ImGui::TableNextColumn()) ImGui::Text("%s", sec->name.c_str());
					if (ImGui::TableNextColumn()) ImGui::Text("%s", sec->inFileBufferName.c_str());
					if (ImGui::TableNextColumn()) ImGui::Text("%i->%i", sec->sourceCodeStartLine, sec->sourceCodeEndLine);
					if (ImGui::TableNextColumn()) ImGui::Text("%s", (sec->isInclude ? "true" : "false"));
					if (ImGui::TableNextColumn()) ImGui::Text("%s", sec->relativeFile.c_str());
		...
		else
		{
			...
			if (ImGui::TableNextColumn()) ImGui::Text("");
			if (ImGui::TableNextColumn()) ImGui::Text("");
			if (ImGui::TableNextColumn()) ImGui::Text("");
			if (ImGui::TableNextColumn()) ImGui::Text("");
			if (ImGui::TableNextColumn()) ImGui::Text("");
			...
			if (ImGui::TableNextColumn()) 
				ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "Start Line Src(%u) Tgt(%u)",
			...
			if (ImGui::TableNextColumn())
				ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "End Line Src(%u) Tgt(%u)",
					vSectionCode->sourceCodeStartLine,
...
void InterfacePanes::DrawDebugPane(CodeTreePtr vCodeTree)
{
	if (vCodeTree)
	{
		if (ImGui::CollapsingHeader("Debug"))
		{
			...
			{
				size_t idx = 0;
				for (auto itSha = vCodeTree->puShaderKeys.begin();
					itSha != vCodeTree->puShaderKeys.end(); itSha++)
				{
					ImGui::Text("Shader : %s", itSha->first.c_str());

					ImGui::Indent();

					for (auto itUni = itSha->second->puUniformsDataBase.begin();
						itUni != itSha->second->puUniformsDataBase.end(); itUni++)
					{
				...
				idx = 0;
				for (auto itInc = vCodeTree->puIncludeKeys.begin();
					itInc != vCodeTree->puIncludeKeys.end(); itInc++)
				{
					ImGui::Text("include : %s", itInc->first.c_str());

					ImGui::Indent();

					for (auto itUni = itInc->second->puUniformsDataBase.begin();
						itUni != itInc->second->puUniformsDataBase.end(); itUni++)
					{
						if (itUni->second)
						{
							if (itUni->first.find(debugSearchFilter) != std::string::npos || debugSearchFilter.empty())
			...
			{
				for (auto it = vCodeTree->puIncludeUniformSectionUniformsList.begin();
					it != vCodeTree->puIncludeUniformSectionUniformsList.end(); ++it)
				{
					...
					for (auto itSec = it->second.begin(); itSec != it->second.end(); ++itSec)
					{
						...
						for (auto itLoc = uniforms->begin(); itLoc != uniforms->end(); ++itLoc)
						{
							...
							if (multiLoc->name.find(debugSearchFilter) != std::string::npos || debugSearchFilter.empty())
							{
								...
								for (auto itMu = multiLoc->linkedUniforms.begin();
									itMu != multiLoc->linkedUniforms.end(); ++itMu)
								{

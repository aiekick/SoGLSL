...
public:
    ...
    virtual void Render(const float& vDeltaTime,
                        UpdateUniformFuncSignature vSpecificUniformsFunc,
                        const DisplayQualityType& vDisplayQuality,
                        MouseInterface* vMouse,
                        const ct::ivec2& vScreenSize,
                        MeshRectType* vMouseRect,
                        GLenum* vRenderMode,
                        FrameBuffersPipeLinePtr vPipe) = 0;
    virtual void DisplayMessageOfRenderPack(const GuiBackend_Window& vWin,

...
bool VRBackend::DrawWidget(CodeTreePtr vCodeTree,
    ...
    if (vUniPtr && vCodeTree && vChange) {
        ...
        if (vUniPtr->widget == "vr:use") {
            ...
            catched = true;
        } else if (vUniPtr->widget == "vr:pos" || vUniPtr->widget == "vr:dir") {
            vCodeTree->DrawUniformName(key, vUniPtr, 0);
            ImGui::SameLine(vFirstColumnWidth);
            ImGui::Text("x:%.3f, y:%.3f, z:%.3f", vUniPtr->x, vUniPtr->y, vUniPtr->z);

            catched = true;
        } else if (vUniPtr->widget == "vr:thumb:left" || vUniPtr->widget == "vr:thumb:right" || vUniPtr->widget == "vr:thumb:accum:left" ||
            ...
            catched = true;
        } else if (vUniPtr->widget == "vr:trigger:left" || vUniPtr->widget == "vr:trigger:right" || vUniPtr->widget == "vr:squeeze:left" ||
...
bool VRBackend::DeSerializeUniform(ShaderKeyPtr vShaderKey, UniformVariantPtr vUniform, const std::vector<std::string>& vParams) {
    UNUSED(vShaderKey);
    UNUSED(vUniform);
    UNUSED(vParams);
...
void VRBackend::Complete_Uniform(ShaderKeyPtr vParentKey, RenderPackWeak vRenderPack, const UniformParsedStruct& vUniformParsed, UniformVariantPtr vUniform) {
...
#undef checkifexist
    ...
    if (!catched) {
        vParentKey->GetSyntaxErrors()->SetSyntaxError(
            vParentKey,
            "Parsing Error :",
            "Bad Uniform Type",
            false,
        ...
        if (vUniPtr->widget == "vr:use") {
            if (m_InRendering) {
                switch (vUniPtr->glslType) {
                    case uType::uTypeEnum::U_FLOAT: vUniPtr->x = 1.0f; return true;
                    case uType::uTypeEnum::U_INT: vUniPtr->ix = 1; return true;
                    case uType::uTypeEnum::U_UINT: vUniPtr->ux = 1U; return true;
                    case uType::uTypeEnum::U_BOOL: vUniPtr->bx = true; return true;
                    default: break;
                }
            } else {
                switch (vUniPtr->glslType) {
                    case uType::uTypeEnum::U_FLOAT: vUniPtr->x = 0.0f; return true;
                    case uType::uTypeEnum::U_INT: vUniPtr->ix = 0; return true;
                    case uType::uTypeEnum::U_UINT: vUniPtr->ux = 0U; return true;
                    case uType::uTypeEnum::U_BOOL: vUniPtr->bx = false; return true;
                    default: break;
                }
            }
...
bool VRBackend::CreateXRInstance() {
    ...
    m_result = xrGetInstanceProcAddr(m_xr_instance, "xrGetOpenGLGraphicsRequirementsKHR", (PFN_xrVoidFunction*)&pfnGetOpenGLGraphicsRequirementsKHR);
    if (XR_FAILED(m_result)) {
...
bool VRBackend::CreateXRViews() {
    ...
    m_result = xrEnumerateViewConfigurationViews(m_xr_instance, m_xr_system_id, m_ViewType, m_ViewCount, &m_ViewCount, m_ViewConfigViews.data());
    if (XR_FAILED(m_result)) {
        auto err = VRUtils::OpenXRResultString(m_result);
...
bool VRBackend::CreateXRSwapChains() {
    ...
    for (uint32_t i = 0; i < m_ViewCount; i++) {
        ...
        m_result = xrEnumerateSwapchainImages(m_SwapChains[i], m_SwapChainsSizes[i], &m_SwapChainsSizes[i], (XrSwapchainImageBaseHeader*)m_SwapChainsImages[i].data());
        if (XR_FAILED(m_result)) {
            auto err = VRUtils::OpenXRResultString(m_result);
    ...
    if (m_OpenXRInfo.depthSupported) {
        ...
        for (uint32_t i = 0; i < m_ViewCount; i++) {
            ...
            m_result = xrEnumerateSwapchainImages(
                m_DepthSwapChains[i], m_DepthSwapChainsSizes[i], &m_DepthSwapChainsSizes[i], (XrSwapchainImageBaseHeader*)m_DepthSwapChainsImages[i].data());
            if (XR_FAILED(m_result)) {
                auto err = VRUtils::OpenXRResultString(m_result);
...
void VRBackend::RetrieveOpenXRInfos() {
    ...
    if (!m_xr_instance_err && !m_xr_system_err) {
        ...
        if (m_xr_system_err)
            LogVarError("No XrSystemId available");
        if (m_xr_instance_err)
            LogVarError("No XrInstance available");
    }
        ...
        switch (runtime_event.type) {
            ...
            case XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED: {
                ...
                LogVarInfo("XR Event : session state changed from %s(%d) to %s(%d)\n",
                           GetSessionStateString(m_xr_session_state),
                           m_xr_session_state,
                           GetSessionStateString(event->state),
                           event->state);
            ...
            default: LogVarError("Unhandled event (type %d)\n", runtime_event.type); break;
        }
...
bool VRBackend::StartXRFrameRendering(uint32_t vViewIndex, GLuint* vCurrentFBOId) {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_SwapChainsImages[vViewIndex][m_AcquiredSwapChainIndex].image, 0);

    if (m_OpenXRInfo.depthSupported) {
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, m_DepthSwapChainsImages[vViewIndex][m_AcquiredSwapChainIndexDepth].image, 0);
    }
...
void VRBackend::RenderFBO(ct::ivec2 vScreenSize, ImVec4 vClearColor) {
    ...
    if (!m_FrameBuffers.empty() && !m_FrameBuffers[0].empty()) {
        if (m_ViewSize.x > 0.0f && m_ViewSize.y > 0.0f) {
            auto rc = ct::GetScreenRectWithSize<float>(ct::fvec2((float)m_ViewSize.x, (float)m_ViewSize.y), ct::fvec2((float)vScreenSize.x, (float)vScreenSize.y), false);
            ...
            glBlitFramebuffer(
                0, 0, (GLint)m_ViewSize.x, (GLint)m_ViewSize.y, (GLint)rc.left, (GLint)rc.bottom, (GLint)rc.right, (GLint)rc.top, GL_COLOR_BUFFER_BIT, GL_LINEAR);
        }
    }
}
...
void VRBackend::ComputeTransforms(const uint32_t vViewIndex) {
    ...
    VRUtils::CreateProjectionFov(uProj, VRUtils::GraphicsAPI::GRAPHICS_OPENGL, m_Views[vViewIndex].fov, camPtr->nearFarPlanes.x, camPtr->nearFarPlanes.y);
    VRUtils::CreateViewMatrix(uModel, m_Views[vViewIndex].pose.position, m_Views[vViewIndex].pose.orientation);

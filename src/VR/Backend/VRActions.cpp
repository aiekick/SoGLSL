...
bool ControllerInput::CreateAction(
	const char *vActionName, 
...
template<typename T>
bool ControllerInput::CaptureDatas(
	const XrSession& vXRSession, 
	const uint32_t& vEye,
	const std::vector<XrPath>& vControllerBasePaths,
	T& vOutputAction)
{
	...
	case XR_ACTION_TYPE_BOOLEAN_INPUT:
		vOutputAction.type = XR_TYPE_ACTION_STATE_BOOLEAN;
		break;
	case XR_ACTION_TYPE_FLOAT_INPUT:
		vOutputAction.type = XR_TYPE_ACTION_STATE_FLOAT;
		break;
	case XR_ACTION_TYPE_VECTOR2F_INPUT:
		vOutputAction.type = XR_TYPE_ACTION_STATE_VECTOR2F;
		break;
	case XR_ACTION_TYPE_POSE_INPUT:
		vOutputAction.type = XR_TYPE_ACTION_STATE_POSE;
		break;
	default:
		break;
	}
	...
	{
	case XR_ACTION_TYPE_BOOLEAN_INPUT:
		result = xrGetActionStateBoolean(vXRSession, &get_info, (XrActionStateBoolean*)&vOutputAction);
		break;
	case XR_ACTION_TYPE_FLOAT_INPUT:
		result = xrGetActionStateFloat(vXRSession, &get_info, (XrActionStateFloat*)&vOutputAction);
		break;
	case XR_ACTION_TYPE_VECTOR2F_INPUT:
		result = xrGetActionStateVector2f(vXRSession, &get_info, (XrActionStateVector2f*)&vOutputAction);
		break;
	case XR_ACTION_TYPE_POSE_INPUT:
		result = xrGetActionStatePose(vXRSession, &get_info, (XrActionStatePose*)&vOutputAction);
		break;
...
bool VRActions::CreateXRActions(const XrInstance& vXRInstance, const XrSession& vXRSession, const XrPosef& vBasePose)
{
	...
	//m_LastControllerDatas.clear();
	//m_LastControllerDatas.resize(HAND_COUNT);
	...
	if (!m_TriggerInput.CreateAction("triggerfloat", "Trigger", m_ControllerBasePaths, m_GamePlayActionSet, XrActionType::XR_ACTION_TYPE_FLOAT_INPUT)) return false;
	if (!m_SqueezeInput.CreateAction("squeezefloat", "Squeeze", m_ControllerBasePaths, m_GamePlayActionSet, XrActionType::XR_ACTION_TYPE_FLOAT_INPUT)) return false;
	if (!m_ThumbstickXInput.CreateAction("thumbstickxfloat", "Thumbstick X", m_ControllerBasePaths, m_GamePlayActionSet, XrActionType::XR_ACTION_TYPE_FLOAT_INPUT)) return false;
	if (!m_ThumbstickYInput.CreateAction("thumbstickyfloat", "Thumbstick Y", m_ControllerBasePaths, m_GamePlayActionSet, XrActionType::XR_ACTION_TYPE_FLOAT_INPUT)) return false;
	...
	actionset_attach_info.type = XR_TYPE_SESSION_ACTION_SETS_ATTACH_INFO,
		actionset_attach_info.next = nullptr;
...
{
	m_ActiveActionSets.resize(1);
	m_ActiveActionSets[0].actionSet = m_GamePlayActionSet;
	m_ActiveActionSets[0].subactionPath = XR_NULL_PATH;

	XrActionsSyncInfo actions_sync_info;
	actions_sync_info.type = XR_TYPE_ACTIONS_SYNC_INFO;
	actions_sync_info.countActiveActionSets = (uint32_t)m_ActiveActionSets.size();
	actions_sync_info.activeActionSets = m_ActiveActionSets.data();
...
void VRActions::GetActionRealTimeInfos(
	const XrSession& vXRSession, 
	const XrSpace& vPlaySpace, 
	const XrFrameState& vFrameState)
{
	...
	for (int i = 0; i < HAND_COUNT; i++)
	{
		...
		m_result = xrLocateSpace(m_ControllerPoseSpaces[i], vPlaySpace,
			vFrameState.predictedDisplayTime, &controllerData.controllerLocation);
		if (XR_FAILED(m_result))
		{
			auto err = VRUtils::OpenXRResultString(m_result);
			...
			controllerData.grabValue.currentState > 0.75f)
		{
			...
			m_result = xrApplyHapticFeedback(vXRSession, &haptic_action_info,
			...
			if (controllerData.squeezeValue.currentState > 0.5f)
			{
				if (i == 0)
				{
					if (CameraSystem::Instance()->m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y)
					{
						if (IS_FLOAT_DIFFERENT(controllerData.thumbstickYValue.currentState, 0.0f))
						{
							...
							CameraSystem::Instance()->IncZoom(
								controllerData.thumbstickYValue.currentState * CameraSystem::Instance()->m_CameraSettings.m_VRZoomStepFactor);
					...
					else if(CameraSystem::Instance()->m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_FREE)
					{
						...
						{
							CameraSystem::Instance()->IncFlyingPosition(controllerData.triggerValue.currentState * CameraSystem::Instance()->m_CameraSettings.m_SpeedFactor * -1.0f);
						}
					}
				}
				else
				{
					...
					else if (CameraSystem::Instance()->m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_FREE)
					{
						...
						if (controllerData.triggerValue.currentState > 0.0f)
						{
							CameraSystem::Instance()->IncFlyingPosition(controllerData.triggerValue.currentState * CameraSystem::Instance()->m_CameraSettings.m_SpeedFactor);
						}
					}
				}
			}
...
void VRActions::EventInteractionProfileChnaged(
	const XrInstance& vXRInstance,
	const XrSession& vXRSession, 
	const XrEventDataBuffer& vRunTimeEvent)
{
	LogVarError("XR Event : interaction profile changed!\n");
	XrEventDataInteractionProfileChanged* event =
		(XrEventDataInteractionProfileChanged*)&vRunTimeEvent;
	(void)event;
...
bool VRActions::SuggestController_Simple(const XrInstance& vXRInstance)
{
	...
	*/
	...
	FillActionPath(vXRInstance,
		"/interaction_profiles/khr/simple_controller", &m_InteractionProfilePath);
	if (XR_FAILED(m_result))
	{
		auto err = VRUtils::OpenXRResultString(m_result);
...
bool VRActions::SuggestController_OculusTouch(const XrInstance& vXRInstance)
{
	...
	*/
	...
	// for simple controller
	FillActionPath(vXRInstance,
		"/interaction_profiles/oculus/touch_controller", &m_InteractionProfilePath);
	if (XR_FAILED(m_result))
	{
		auto err = VRUtils::OpenXRResultString(m_result);
...
bool VRActions::SuggestController_ValveIndex(const XrInstance& vXRInstance)
{
	...
	*/
	...
	// for simple controller
	FillActionPath(vXRInstance,
		"/interaction_profiles/valve/index_controller", &m_InteractionProfilePath);
	if (XR_FAILED(m_result))
	{
		auto err = VRUtils::OpenXRResultString(m_result);

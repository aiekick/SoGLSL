		...
		if (strName == "cullingmatrix")
		{
			...
			if (vec.size() == 3)
			{
				if (puGizmoCullingUniform)
				{
					...
					if (vecMat.size() == 16)
					{
						puGizmoCullingUniform->mat4 = glm::mat4(
							vecMat[0], vecMat[1], vecMat[2], vecMat[3],
							vecMat[4], vecMat[5], vecMat[6], vecMat[7],
							vecMat[8], vecMat[9], vecMat[10], vecMat[11],
							vecMat[12], vecMat[13], vecMat[14], vecMat[15]);
					}
	...
	const bool& /*vShowUnUsed*/, const bool& /*vShowCustom*/, const bool& vForNodes, bool* vChange)
{
	...
	if (vUniPtr && vCodeTree && vChange)
	{
		...
		else if (vUniPtr->widget == "culling")
		{
			...
			if (v->loc > -1) ImGui::Text("Matrix 4x4");
			else  ImGui::Text("Not Used");
			...
			if (ImGui::IsItemActive() || ImGui::IsItemHovered())
			{
				...
				if (v->uFloatArr)
				{
					ImGui::Text("0 : %.2f %.2f %.2f %.2f\n1 : %.2f %.2f %.2f %.2f\n2 : %.2f %.2f %.2f %.2f\n3 : %.2f %.2f %.2f %.2f",
						v->uFloatArr[0], v->uFloatArr[1], v->uFloatArr[2], v->uFloatArr[3],
						v->uFloatArr[4], v->uFloatArr[5], v->uFloatArr[6], v->uFloatArr[7],
						v->uFloatArr[8], v->uFloatArr[9], v->uFloatArr[10], v->uFloatArr[11],
						v->uFloatArr[12], v->uFloatArr[13], v->uFloatArr[14], v->uFloatArr[15]);
				}
				else
				{
					ImGui::Text("0 : %.2f %.2f %.2f %.2f\n1 : %.2f %.2f %.2f %.2f\n2 : %.2f %.2f %.2f %.2f\n3 : %.2f %.2f %.2f %.2f",
						v->mat4[0][0], v->mat4[1][0], v->mat4[2][0], v->mat4[3][0],
						v->mat4[0][1], v->mat4[1][1], v->mat4[2][1], v->mat4[3][1],
						v->mat4[0][2], v->mat4[1][2], v->mat4[2][2], v->mat4[3][2],
						v->mat4[0][3], v->mat4[1][3], v->mat4[2][3], v->mat4[3][3]);
				}
...
bool GizmoSystem::DeSerializeUniform(ShaderKeyPtr  /*vShaderKey*/, UniformVariantPtr vUniform, const std::vector<std::string>& vParams)
{
	...
	if (vUniform->widget == "gizmo")
	{
		if (vParams.size() == 4)
		{
			...
			if (vec.size() == 16)
			{
				vUniform->mat4 = glm::mat4(vec[0], vec[1], vec[2], vec[3], vec[4], vec[5], vec[6], vec[7], vec[8], vec[9], vec[10], vec[11], vec[12], vec[13], vec[14], vec[15]);
			}
...
void GizmoSystem::Complete_Uniform(ShaderKeyPtr vParentKey, RenderPackWeak vRenderPack, const UniformParsedStruct& vUniformParsed, UniformVariantPtr vUniform)
{
	...
	if (vUniform->widgetType == "useculling" || 
		vUniform->widgetType == "culling" ||
		vUniform->widgetType == "cullingtype")
	{
...
void GizmoSystem::Complete_Uniform_Culling(ShaderKeyPtr  /*vParentKey*/, RenderPackWeak vRenderPack, const UniformParsedStruct&
                                           /*vUniformParsed*/, UniformVariantPtr vUniform)
{
    if (!vRenderPack.expired())
		return;
...
std::string GizmoSystem::InitRenderPack(const GuiBackend_Window& vWin, CodeTreePtr vCodeTree)
{
	if (vCodeTree)
	{
		...
		if (path.empty()) CreateFilePathName("predefined/helpers/culling.glsl", "culling", false);
		...
		puGizmoCulling_RenderPack = RenderPack::createBufferWithFileWithoutLoading(
			vWin,
			"GizmoCulling",
			ct::ivec3(0),
			puGizmoCulling_Key,
...
void main(void)
{
	...
	if (!file.empty())
	{
		FileHelper::Instance()->SaveToFile(file, vFilePathName, useStandardPaths ? (int)FILE_LOCATION_Enum::FILE_LOCATION_SCRIPT : (int)FILE_LOCATION_Enum::FILE_LOCATION_NONE);

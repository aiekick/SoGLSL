    ...
    if (vMessagePtr && vUserDataPtr) {
        ...
        if (port < m_MidiDevices.size()) {
            if (!vMessagePtr->empty()) {
                ...
                m_MidiDevices[port].currentMessage.name = m_MidiDevices[port].deviceName, m_MidiDevices[port].currentMessage.bytes = *vMessagePtr;
...
bool MidiSystem::DrawWidget(CodeTreePtr vCodeTree,
...
void MidiSystem::Complete_Uniform(ShaderKeyPtr vParentKey, RenderPackWeak vRenderPack, const UniformParsedStruct& vUniformParsed, UniformVariantPtr vUniform) {
    ...
    if (!catched) {
        vParentKey->GetSyntaxErrors()->SetSyntaxError(vParentKey,
                                                      "Parsing Error :",
                                                      "Bad Uniform Type",
                                                      false,
        ...
        if (m_RtMidis[i]->isPortOpen()) {
            ...
            if (!m_MidiDevices[i].currentMessage.bytes.empty()) {
                ...
                const std::string markdownText =
                    u8R"(# Auto Detection of Uniform Syntax :
...
[uniform](#00FF0B) [float](#00C9FF)([midi](#E2FF00):id=)" +
                    id + u8R"(:value=)" + value + u8R"(:0.0:1.0:0.5:0.0) inputName)";
                Markdown(markdownText);

                if (ImGui::ContrastedButton("Copy midi id+value to clipboard")) {
                    GuiBackend::Instance()->SetClipboardString(vWin, ct::toStr("midi:id=%s:value=%s", id.c_str(), value.c_str()));
                }

    ...
    if (vType != BaseMeshEnum::PRIMITIVE_TYPE_NONE && !vFilePathName.empty()) {
        ...
        if (!path.empty()) {
            ...
            if (prRecentFilesForDiplay.size() > 1U) {
                std::sort(
                    prRecentFilesForDiplay.begin(), prRecentFilesForDiplay.end(), [](const std::weak_ptr<RecentFilesInfos>& a, const std::weak_ptr<RecentFilesInfos>& b) {
                        auto ptr_a = a.lock();
                        auto ptr_b = b.lock();
                        if (!ptr_a || !ptr_b)
                            return false;
                ...
                for (const auto& block : prRecentFilesForDiplay) {
                    ...
                    if (ptr_block.use_count()) {
                        ...
                        switch (ptr_block->type) {
                            case BaseMeshEnum::PRIMITIVE_TYPE_QUAD: type = "Quad"; break;
                            case BaseMeshEnum::PRIMITIVE_TYPE_POINTS: type = "Points"; break;
                            case BaseMeshEnum::PRIMITIVE_TYPE_MESH: type = "Mesh"; break;
                            default: break;
                        }
...
std::string RecentFilesSystem::getXml(const std::string& vOffset, const std::string& vUserDatas) {
    ...
    for (const auto& block : prRecentFilesForCalc) {
        if (block.second.use_count())
            str += offset + "<file type=\"" + ct::toStr((int)block.second->type) + "\" stamp=\"" + ct::toStr(block.second->timestamp) + "\" path=\"" +
                block.second->path + "\"/>\n";
    }

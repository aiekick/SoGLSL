...
bool CameraSystem::DrawImGui() {
    ...
    if (ImGui::CollapsingHeader("Camera")) {
        ...
        if (ImGui::RadioButtonLabeled(
                ImVec2(aw, 0.0f), "Perspective", "Use Perspective Camera", m_CameraSettings.m_CameraType == CAMERA_TYPE_Enum::CAMERA_TYPE_PERSPECTIVE)) {
        ...
        if (ImGui::RadioButtonLabeled(
                ImVec2(aw, 0.0f), "Orthographic", "Use Orthographic Camera", m_CameraSettings.m_CameraType == CAMERA_TYPE_Enum::CAMERA_TYPE_ORTHOGRAPHIC)) {
        ...
        if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_FREE) {
            ...
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Cam Pos X##campx", &m_CameraSettings.m_PosXYZ.x, -10.0f, 10.0f, s_CameraSettingsStruct_Default.m_PosXYZ.x);
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Cam Pos Y##campy", &m_CameraSettings.m_PosXYZ.y, -10.0f, 10.0f, s_CameraSettingsStruct_Default.m_PosXYZ.y);
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Cam Pos Z##campz", &m_CameraSettings.m_PosXYZ.z, -10.0f, 10.0f, s_CameraSettingsStruct_Default.m_PosXYZ.z);
            change |=
                ImGui::SliderFloatDefaultCompact(-1.0f, "Speed##camspeed", &m_CameraSettings.m_SpeedFactor, 0.0f, 100.0f, s_CameraSettingsStruct_Default.m_SpeedFactor);
        }

        if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y) {
            ImGui::FramedGroupSeparator();

            ImGui::CheckBoxIcon("##lock_target_x", ICON_NDP2_LOCK, &m_CameraSettings.m_TargetXYZLock.x);
            ImGui::SameLine();
            ...
            ImGui::CheckBoxIcon("##lock_target_y", ICON_NDP2_LOCK, &m_CameraSettings.m_TargetXYZLock.y);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
            ...
            ImGui::CheckBoxIcon("##lock_target_z", ICON_NDP2_LOCK, &m_CameraSettings.m_TargetXYZLock.z);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
            ...
            ImGui::CheckBoxIcon("##lock_trans_x", ICON_NDP2_LOCK, &m_CameraSettings.m_TransXYLock.x);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
            ...
            ImGui::CheckBoxIcon("##lock_trans_y", ICON_NDP2_LOCK, &m_CameraSettings.m_TransXYLock.y);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
            ...
            change |=
                ImGui::SliderFloatDefaultCompact(-1.0f, "Factor##trans", &m_CameraSettings.m_TransFactor, 0.0f, 100.0f, s_CameraSettingsStruct_Default.m_TransFactor);
        }

        ImGui::FramedGroupSeparator();

        ImGui::CheckBoxIcon("##lock_rot_x", ICON_NDP2_LOCK, &m_CameraSettings.m_RotationXYZLock.x);
        ImGui::SameLine();
        if (ImGui::IsItemHovered())
        ...
        ImGui::CheckBoxIcon("##lock_rot_y", ICON_NDP2_LOCK, &m_CameraSettings.m_RotationXYZLock.y);
        ImGui::SameLine();
        if (ImGui::IsItemHovered())
        ...
        ImGui::CheckBoxIcon("##lock_rot_z", ICON_NDP2_LOCK, &m_CameraSettings.m_RotationXYZLock.z);
        ImGui::SameLine();
        if (ImGui::IsItemHovered())
        ...
        // ImGui::FramedGroupSeparator();
        // ImGui::CheckBoxIcon("##lock_uni_scale", ICON_NDP2_LOCK, &m_CameraSettings.m_UniformScaleLock); ImGui::SameLine(); if (ImGui::IsItemHovered())
        // ImGui::SetTooltip("lock scale"); change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Scale##scale", &m_CameraSettings.m_UniformScale, 0.00001f, 2.0f, 1.0f);

        if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y) {
            ImGui::FramedGroupSeparator();

            ImGui::CheckBoxIcon("##lock_zoom", ICON_NDP2_LOCK, &m_CameraSettings.m_ZoomLock);
            ImGui::SameLine();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("lock zoom");
            change |=
                ImGui::SliderFloatDefaultCompact(-1.0f, "Zoom##Zoom", &m_CameraSettings.m_Zoom, 0.00001f, 100.0f, s_CameraSettingsStruct_Default.m_Zoom, 0.0f, "%.5f");
            change |= ImGui::SliderFloatDefaultCompact(-1.0f, "Factor##Zoom", &m_CameraSettings.m_ZoomFactor, 0.0f, 100.0f, s_CameraSettingsStruct_Default.m_ZoomFactor);
        }
        ...
        if (VRBackend::Instance()->IsLoaded()) {
            ...
            ImGui::CheckBoxBoolDefault("Use VR Controller for control Camera",
                                       ...
                                       "if left or right Squeeze are pressed");
            if (m_CameraSettings.m_UseVRControllerForControlCamera) {
                change |= ImGui::SliderFloatDefaultCompact(-1.0f,
                                                           "VR Step Rot X##vrrotx",
                                                           &m_CameraSettings.m_VRRotStepFactor.x,
                                                           0.00001f,
                                                           2.0f,
                                                           s_CameraSettingsStruct_Default.m_VRRotStepFactor.x,
                                                           0.0f,
                                                           "%.5f");
                change |= ImGui::SliderFloatDefaultCompact(-1.0f,
                                                           "VR Step Rot Y##vrroty",
                                                           &m_CameraSettings.m_VRRotStepFactor.y,
                                                           0.00001f,
                                                           2.0f,
                                                           s_CameraSettingsStruct_Default.m_VRRotStepFactor.y,
                                                           0.0f,
                                                           "%.5f");
                change |= ImGui::SliderFloatDefaultCompact(-1.0f,
                                                           "VR Step Rot Z##vrrotz",
                                                           &m_CameraSettings.m_VRRotStepFactor.z,
                                                           0.00001f,
                                                           2.0f,
                                                           s_CameraSettingsStruct_Default.m_VRRotStepFactor.z,
                                                           0.0f,
                                                           "%.5f");
                change |= ImGui::SliderFloatDefaultCompact(
                    -1.0f, "VR Step Zoom##vrzoom", &m_CameraSettings.m_VRZoomStepFactor, 0.00001f, 5.0f, s_CameraSettingsStruct_Default.m_VRZoomStepFactor, 0.0f, "%.5f");
            }
        }
#endif
...
glm::mat4 CameraSystem::ComputeModelMatrix() {
    ...
    if (m_CameraSettings.m_CameraMode == CAMERA_MODE_Enum::CAMERA_MODE_TURNTABLE_Y) {
        ...
        Model = glm::translate(glm::mat4_cast(camOrientation), glm::vec3(m_CameraSettings.m_PosXYZ.x, m_CameraSettings.m_PosXYZ.y, m_CameraSettings.m_PosXYZ.z));
    }

                ...
                if (!m_AudioDevices.empty()) {
                    ...
                    for (size_t i = 0U; i < current_device.nativeDataFormatCount; ++i) {
                        ...
                        switch (data_format.format) {
                            case ma_format_unknown: ImGui::Text("Type : Unknow"); break;
                            case ma_format_u8: ImGui::Text("Type : U8"); break;
                            case ma_format_s16: ImGui::Text("Type : S16"); break;
                            case ma_format_s24: ImGui::Text("Type : S24"); break;
                            case ma_format_s32: ImGui::Text("Type : S32"); break;
                            case ma_format_f32: ImGui::Text("Type : F32"); break;
    ...
    if (vUniPtr) {
        if (vUniPtr->widgetType == "sound") {
            ...
            if (vUniPtr->sound_ptr) {
                ...
                if (vUniPtr->uSampler1D > -1 && vUniPtr->loc > -1) {
            ...
            if (m_SoundHisto_RenderPack_Ptr) {
                ...
                if (vUniPtr->sound_histo_ptr) {
                    ...
                    if (vUniPtr->uSampler2D > -1 && vUniPtr->loc > -1) {
                        glActiveTexture(GL_TEXTURE0 + vTextureSlotId);
...
bool SoundSystem::DrawWidget(CodeTreePtr vCodeTree,
    ...
    if (vUniPtr && vCodeTree && vChange) {
        ...
        if (v->widgetType == "sound_histo" && m_SoundHisto_RenderPack_Ptr) {
            catched = true;
            ImGui::Separator();
...
void SoundSystem::Complete_Uniform_Sound_With_Sound(ShaderKeyPtr vParentKeyPtr,
                                                    RenderPackWeak vRenderPack,
...
std::string SoundSystem::InitRenderPack(const GuiBackend_Window& vWin, CodeTreePtr vCodeTree) {
    if (vCodeTree) {
        ...
        m_SoundHisto_RenderPack_Ptr =
            RenderPack::createBufferWithFileWithoutLoading(vWin, "SoundHisto", ct::ivec3(SoundSystem::scFftSize, 240U, 0), m_SoundHisto_Key_Ptr, false, true);
    }
...
void SoundSystem::AddFrames(const uint32_t& vFrameCount, const void* vInputPtr) {
    ...
    for (uint32_t i = 0; i < frameCount; ++i) {
        ...
        *(p++) = (samples[i * 2] + samples[i * 2 + 1]) * 0.5f * m_AudioFloatAmplification * (float)BlackmanWindow(i);
    }
}
...
bool SoundSystem::ComputeFFT() {
    if (Use() && m_IsAudioCaptureReady && m_IsNewCaptureDataAvailable) {
...
void SoundSystem::Update() {
    ...
    if (frame_to_capture_count && frame++ > frame_to_capture_start) {
    ...
    if (Use() && m_IsAudioCaptureReady) {
        ...
        if (last_device_state != device_state || m_NeedNewDeviceCheck) {
            switch (device_state) {
                case ma_device_state_uninitialized: LogVarInfo("Currente Device is not initialized"); break;
                case ma_device_state_stopped: LogVarInfo("Currente Device is stopped"); break;
                case ma_device_state_started: LogVarInfo("Currente Device is started"); break;
                case ma_device_state_starting: LogVarInfo("Currente Device is starting"); break;
                case ma_device_state_stopping: LogVarInfo("Currente Device is stopping"); break;
            }

            if (device_state == ma_device_state_stopping || device_state == ma_device_state_stopped || device_state == ma_device_state_uninitialized) {
...
void SoundSystem::Render(float vDeltaTime) {
    if (Use() && m_IsNewCaptureDataAvailable) {
        ...
        if (m_SoundHisto_RenderPack_Ptr) {
            ...
            m_SoundHisto_RenderPack_Ptr->UpdateUniforms(std::bind(&SoundSystem::UpdateHistoRenderPackUniforms,
                                                                  this,
                                                                  std::placeholders::_1,
                                                                  std::placeholders::_2,
                                                                  std::placeholders::_3,
...
ctTexturePtr SoundSystem::CreateR32FTexture1D(const size_t& vWidth) {
    ...
    if (verifSize > 0) {
        ...
        glTexImage1D(GL_TEXTURE_1D, 0, res->glinternalformat, (GLsizei)res->w, 0, res->glformat, res->gldatatype, nullptr);
...
ctTexturePtr SoundSystem::CreateR32FTexture2D(const size_t& vWidth, const size_t& vHeight) {
    ...
    if (verifSize > 0) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, res->glinternalformat, (GLsizei)res->w, (GLsizei)res->h, 0, res->glformat, res->gldatatype, nullptr);
...
void SoundSystem::UpdateR32FTexture1D(ctTexturePtr vTexturePtr, const float* vSamplesPtr) {
    if (vTexturePtr && vSamplesPtr && vTexturePtr->glTextureType == GL_TEXTURE_1D && vTexturePtr->glTex) {
...
void SoundSystem::UpdateR32FTexture2D(ctTexturePtr vTexturePtr, const float* vSamplesPtr) {
    if (vTexturePtr && vSamplesPtr && vTexturePtr->glTextureType == GL_TEXTURE_2D && vTexturePtr->glTex) {

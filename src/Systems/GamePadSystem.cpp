...
bool GamePadSystem::DrawWidget(CodeTreePtr vCodeTree,
...
void GamePadSystem::Complete_Uniform(ShaderKeyPtr vParentKey, RenderPackWeak vRenderPack, const UniformParsedStruct& vUniformParsed, UniformVariantPtr vUniform) {
    ...
    if (!catched) {
        vParentKey->GetSyntaxErrors()->SetSyntaxError(vParentKey,
                                                      ...
                                                      "Bad Uniform Type",
                                                      false,
                                                      LineFileErrors(vUniformParsed.sourceCodeLine,
                                                                     "",
                                                                     "uniform of type gamepad can only be :\n - vec3 for left and right thumb (-1 to 1)\n - float for "
                                                                     "left and right trigger (0 to 1)\n - vec4 for cross and main buttons (true 1.0 or false 0.0)"));
    }
}

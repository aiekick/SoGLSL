...
FrameBuffersPipeLinePtr FrameBuffersPipeLine::Create(const GuiBackend_Window& vWin,
    ...
    if (!vSize.emptyAND() && vCountTextures > 0) {
        pipeline =
            std::make_shared<FrameBuffersPipeLine>(vWin, vSize, vDataType, vUseFXAA, vCountFXAASamples, vUseZBuffer, vCountTextures, vUseRenderBuffer, vUseFloatBuffer);
...
FrameBuffersPipeLine::FrameBuffersPipeLine(const GuiBackend_Window& vWin,
                                           const ct::ivec3& vSize,
                                           const std::string& vDataType,
                                           const bool& vUseFXAA,
                                           const int& vCountFXAASamples,
...
bool FrameBuffersPipeLine::AddColorAttachment(TextureParamsStruct* vTexParam) {
    const bool res = puFrontBuffer->AddColorAttachment(vTexParam) && puBackBuffer->AddColorAttachment(vTexParam);
    glFinish();
    LogGlError();
    UpdatePipeline();
    glFinish();
...
bool FrameBuffersPipeLine::RemoveColorAttachment(const size_t& vId) {
    const bool res = puFrontBuffer->RemoveColorAttachment(vId) && puBackBuffer->RemoveColorAttachment(vId);
    glFinish();
    LogGlError();
    UpdatePipeline();
    glFinish();
...
bool FrameBuffersPipeLine::Resize(const ct::ivec2& vNewSize) {
    ...
    if (puFrontBuffer && puBackBuffer) {
        res = puFrontBuffer->Resize(vNewSize) && puBackBuffer->Resize(vNewSize);
    }

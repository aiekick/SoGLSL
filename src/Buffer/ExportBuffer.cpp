...
void ExportBuffer::AfterCapture(const GLuint& vRenderMode)
{
	...
	if (vRenderMode == GL_TRIANGLES || 
		vRenderMode == GL_PATCHES)
	{
...
void ExportBuffer::Capture(BaseModelWeak vModel, const GLuint& vRenderMode)
{
	...
	if (modelPtr != nullptr)
	{
		...
		}
		{
			...
			{
				glDrawElementsInstanced(vRenderMode, (GLsizei)modelPtr->GetIndicesCountToShow(), GL_UNSIGNED_INT, nullptr,
					(GLsizei)modelPtr->GetInstancesCount());
				LogGlError();
			}
			else
			{

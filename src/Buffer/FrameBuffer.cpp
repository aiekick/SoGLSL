	...
	const bool& vUseFloatBuffer)
{
	...
	if (!vSize.emptyAND() && vCountTextures > 0)
	{
		fbo = std::make_shared<FrameBuffer>(
			vWin, vSize, vDataType, vUseFXAA, vCountFXAASamples, 
			vUseZBuffer, vCountTextures, vUseRenderBuffer, vUseFloatBuffer);
	}
...
void FrameBuffer::SetFormatFromString(const std::string& vDataTypeString, GLenum *vInternalFormat, GLenum *VFormat, GLenum *vDataType)
{
	...
	{
		if (vInternalFormat) *vInternalFormat = GL_RGBA32F;
		if (VFormat) *VFormat = GL_RGBA;
		if (vDataType) *vDataType = GL_FLOAT;
	}
	else if (vDataTypeString == "byte")
	{
		if (vInternalFormat) *vInternalFormat = GL_RGBA;
		if (VFormat) *VFormat = GL_RGBA;
		if (vDataType) *vDataType = GL_UNSIGNED_BYTE;
	}
}
...
std::shared_ptr<FloatBuffer> FrameBuffer::GetFloatBufferFromColorAttachment_4_Chan(
	const int& vAttachmentId, const bool& vReadFBO, const int& vMipMapLvl, const bool& vCreateNewBuffer)
{
...
std::shared_ptr<FloatBuffer> FrameBuffer::GetFloatBufferFromColorAttachment_1_Chan(
	const int& vAttachmentId, const bool& vReadFBO, const int& vMipMapLvl, const bool& vCreateNewBuffer)
{
	std::shared_ptr<FloatBuffer> fBuffer = nullptr;
...
uint8_t* FrameBuffer::GetRGBBytesFromFrameBuffer(
	int *vWidth, int *vHeight, int *vBufSize, const int& vAttachmentId)
{
	...
	if (bmFloatRGBA)
	{
		...
		if ((vNewSize.x != width) || (vNewSize.y != height))
		{
			...
			if (resizeRes)
			{
				if (vFlipY)
					stbi_flip_vertically_on_write(1);
				const int resWrite = stbi_write_hdr(vFilePathName.c_str(),
				                                    newWidth,
				                                    newHeight,
				                                    bytesPerPixel,
				                                    resizedData);

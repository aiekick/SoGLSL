    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_QUIT) {
            ...
            if (event.button.button == SDL_BUTTON_LEFT) {
                m_ButtonsDown[0] = true;
            } else if (event.button.button == SDL_BUTTON_RIGHT) {
                m_ButtonsDown[1] = true;
            } else if (event.button.button == SDL_BUTTON_MIDDLE) {
                m_ButtonsDown[2] = true;
            }
        } else if (event.type == SDL_MOUSEBUTTONUP) {
        } else if (event.type == SDL_WINDOWEVENT) {
            if (event.window.event == SDL_WINDOWEVENT_CLOSE && event.window.windowID == SDL_GetWindowID(vWindow.win)) {
                res = true;
            } else {
                switch (event.window.event) {
                    case SDL_WINDOWEVENT_SHOWN: LogVarDebug("Window %d shown", event.window.windowID); break;
                    case SDL_WINDOWEVENT_HIDDEN: LogVarDebug("Window %d hidden", event.window.windowID); break;
                    case SDL_WINDOWEVENT_EXPOSED: LogVarDebug("Window %d exposed", event.window.windowID); break;
                    case SDL_WINDOWEVENT_MOVED: LogVarDebug("Window %d moved to %d,%d", event.window.windowID, event.window.data1, event.window.data2); break;
                    case SDL_WINDOWEVENT_RESIZED: LogVarDebug("Window %d resized to %dx%d", event.window.windowID, event.window.data1, event.window.data2); break;
                    case SDL_WINDOWEVENT_SIZE_CHANGED:
                        LogVarDebug("Window %d size changed to %dx%d", event.window.windowID, event.window.data1, event.window.data2);
                        break;
                    case SDL_WINDOWEVENT_MINIMIZED: LogVarDebug("Window %d minimized", event.window.windowID); break;
                    case SDL_WINDOWEVENT_MAXIMIZED: LogVarDebug("Window %d maximized", event.window.windowID); break;
                    case SDL_WINDOWEVENT_RESTORED: LogVarDebug("Window %d restored", event.window.windowID); break;
                    case SDL_WINDOWEVENT_ENTER: LogVarDebug("Mouse entered window %d", event.window.windowID); break;
                    case SDL_WINDOWEVENT_LEAVE: LogVarDebug("Mouse left window %d", event.window.windowID); break;
                    case SDL_WINDOWEVENT_FOCUS_GAINED:
                        ...
                        LogVarDebug("Window %d gained keyboard focus", event.window.windowID);
                        break;
                    case SDL_WINDOWEVENT_FOCUS_LOST:
                        ...
                        LogVarDebug("Window %d lost keyboard focus", event.window.windowID);
                        break;
                    case SDL_WINDOWEVENT_CLOSE: LogVarDebug("Window %d closed", event.window.windowID); break;
#if SDL_VERSION_ATLEAST(2, 0, 5)
                    case SDL_WINDOWEVENT_TAKE_FOCUS: LogVarDebug("Window %d is offered a focus", event.window.windowID); break;
                    case SDL_WINDOWEVENT_HIT_TEST: LogVarDebug("Window %d has a special hit test", event.window.windowID); break;
#endif
                    default: LogVarDebug("Window %d got unknown event %d", event.window.windowID, event.window.event); break;
                }
            }
        }
        CheckForError();

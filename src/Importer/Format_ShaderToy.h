...
/* "Shader":
{
    ...
    "info" :
    {
        ...
        "username" : "aiekick",
        "description" : "i have the limit of the float after a certain time, and some noise appears :)\nyou can increase the iteration at line 86 to have better shape but
more slow too :) ", "likes" : 28, "published" : 3, "flags" : 0, "tags" : ["tunnelbeauty6"], "hasliked" : 0
    },
    "renderpass":
    [{
        "inputs":
...
pow(clamp( dot( refl, lid ), 0.0, 1.0 ),16.);\t\t\/\/ specular\n\tvec3 col = vec3(.8,.5,.2);\n    \n    \/\/ here the magic happen\n\tfloat sss = df(p -
n*0.001)\/0.1;\t\t\t\t\t\t\t\t\/\/ quick sss 0.001 of subsurface\n\t\n\tfloat sb = SubDensity(p, 0.01, 0.1);\t\t\t\t\t\t\t\/\/ deep subdensity from 0.01 to 0.1 (10
iterations)\n\tvec3 bb = clamp(blackbody(200. * sb),0.,1.);\t\t\t\t\t\/\/ blackbody color\n\tfloat sss2 = 0.8 - SubDensity(p, 3.); \t\t\t\t\t\t\t\/\/ one step sub density
of df of 3 of subsurface\n\t\n    fragColor.rgb = (diff + fre + bb * sss2 * .8 + col * sss * .2) * 0.25 + spe * 1.2;\n\n\t\/\/ vigneting from iq Shader Mike :
...
"Shader":
{
    ...
    "info":
    {
        ...
        "username":"Flexi",
        "description":"Multi-scale \"Milkdrop2\" Gaussian blur diffusion, mouse drag and drop vortex pair plane deformation, cyclic rgba reaction, gradient lookups for
expansive flow and edgy color map", "likes":165, "published":3, "flags":32, "tags":
        [
            "multiscale",
            "reactiondiffusion"
        ],
...
upper_left(vec2 uv)\n{\n    return fract((uv - vec2(0., 1)) * 0.5);\n}\n\nvec2 upper_right(vec2 uv)\n{\n    return fract((uv - 1.) * 0.5);\n}\n\nvec4 BlurA(vec2 uv, int
level)\n{\n    if(level <= 0)\n    {\n        return texture2D(iChannel0, fract(uv));\n    }\n\n    uv = upper_left(uv);\n    for(int depth = 1; depth < 8; depth++)\n {\n
if(depth >= level)\n        {\n            break;\n        }\n        uv = lower_right(uv);\n    }\n\n    return texture2D(iChannel3, uv);\n}\n\nvec4 BlurB(vec2 uv, int
level)\n{\n    if(level <= 0)\n    {\n        return texture2D(iChannel1, fract(uv));\n    }\n\n    uv = lower_left(uv);\n    for(int depth = 1; depth < 8; depth++)\n {\n
if(depth >= level)\n        {\n            break;\n        }\n        uv = lower_right(uv);\n    }\n\n    return texture2D(iChannel3, uv);\n}\n\nvec2 GradientA(vec2 uv,
vec2 d, vec4 selector, int level){\n    vec4 dX = 0.5*BlurA(uv + vec2(1.,0.)*d, level) - 0.5*BlurA(uv - vec2(1.,0.)*d, level);\n    vec4 dY = 0.5*BlurA(uv +
vec2(0.,1.)*d, level) - 0.5*BlurA(uv - vec2(0.,1.)*d, level);\n    return vec2( dot(dX, selector), dot(dY, selector) );\n}\n\nvoid mainImage( out vec4 fragColor, in vec2
fragCoord )\n{\n    vec2 uv = fragCoord.xy \/ iResolution.xy;\n    vec2 pixelSize = 1. \/ iResolution.xy;\n    vec2 aspect = vec2(1.,iResolution.y\/iResolution.x);\n\n
vec2 d = pixelSize*2.;\n    vec4 dx = (BlurA(uv + vec2(1,0)*d, 1) - BlurA(uv - vec2(1,0)*d, 1))*0.5;\n    vec4 dy = (BlurA(uv + vec2(0,1)*d, 1) - BlurA(uv - vec2(0,1)*d,
1))*0.5;\n\n    d = pixelSize*1.;\n    dx += BlurA(uv + vec2(1,0)*d, 0) - BlurA(uv - vec2(1,0)*d, 0);\n    dy += BlurA(uv + vec2(0,1)*d, 0) - BlurA(uv - vec2(0,1)*d,
0);\n    vec2 lightSize=vec2(0.5);\n\n    fragColor = BlurA(uv+vec2(dx.x,dy.x)*pixelSize*8., 0).x * vec4(0.7,1.66,2.0,1.0) - vec4(0.3,1.0,1.0,1.0);\n    fragColor =
mix(fragColor,vec4(8.0,6.,2.,1.), BlurA(uv + vec2(dx.x,dy.x)*lightSize, 3).y*0.4*0.75*vec4(1.-BlurA(uv+vec2(dx.x,dy.x)*pixelSize*8., 0).x)); \n    fragColor =
mix(fragColor, vec4(0.1,0.,0.4,0.), BlurA(uv, 1).a*length(GradientA(uv, pixelSize*2., vec4(0.,0.,0.,1.), 0))*5.);\n    fragColor = mix(fragColor, vec4(1.25,1.35,1.4,0.),
BlurA(uv, 0).x*BlurA(uv + GradientA(uv, pixelSize*2.5, vec4(-256.,32.,-128.,32.), 1)*pixelSize, 2).y);\n    fragColor = mix(fragColor, vec4(0.25,0.75,1.,0.), BlurA(uv,
1).x*length(GradientA(uv+GradientA(uv, pixelSize*2., vec4(0.,0.,128.,0.), 1)*pixelSize, pixelSize*2., vec4(0.,0.,0.,1.), 0))*5.);\n    fragColor = mix(fragColor,
vec4(1.,1.25,1.5,0.), 0.5*(1.-BlurA(uv, 0)*1.).a*length(GradientA(uv+GradientA(uv, pixelSize*2., vec4(0.,128.,0.,0.), 1)*pixelSize, pixelSize*1.5, vec4(0.,0.,16.,0.),
0)));\n\n    \/\/    fragColor = BlurA(uv, 0); \/\/ simple bypass\n    \/\/    fragColor = BlurB(uv, 0); \/\/ simple bypass\n    \/\/    fragColor = texture2D(iChannel3,
uv); \/\/ raw Gaussian pyramid\n\n}", "name":"Image", "description":"", "type":"image"
        },
        {
            "inputs":
            [
...
domain - center;\n    float d = length(uv);\n    return vec2( atan(uv.y, uv.x)*n*pi2_inv + d*spiral_factor, -log(d)*zoopufactor) + pos;\n}\n\nvec2 complex_div(vec2
numerator, vec2 denominator){\n    return vec2( numerator.x*denominator.x + numerator.y*denominator.y,\n                numerator.y*denominator.x -
numerator.x*denominator.y)\/\n        vec2(denominator.x*denominator.x + denominator.y*denominator.y);\n}\n\nfloat circle(vec2 uv, vec2 aspect, float scale){\n    return
clamp( 1. - length((uv-0.5)*aspect*scale), 0., 1.);\n}\n\nfloat sigmoid(float x) {\n    return 2.\/(1. + exp2(-x)) - 1.;\n}\n\nfloat smoothcircle(vec2 uv, vec2 aspect,
float radius, float ramp){\n    return 0.5 - sigmoid( ( length( (uv - 0.5) * aspect) - radius) * ramp) * 0.5;\n}\n\nfloat conetip(vec2 uv, vec2 pos, float size, float
min)\n{\n    vec2 aspect = vec2(1.,iResolution.y\/iResolution.x);\n    return max( min, 1. - length((uv - pos) * aspect \/ size) );\n}\n\nfloat warpFilter(vec2 uv, vec2
pos, float size, float ramp)\n{\n    return 0.5 + sigmoid( conetip(uv, pos, size, -16.) * ramp) * 0.5;\n}\n\nvec2 vortex_warp(vec2 uv, vec2 pos, float size, float ramp,
vec2 rot)\n{\n    vec2 aspect = vec2(1.,iResolution.y\/iResolution.x);\n\n    vec2 pos_correct = 0.5 + (pos - 0.5);\n    vec2 rot_uv = pos_correct + complex_mul((uv -
pos_correct)*aspect, rot)\/aspect;\n    float filter = warpFilter(uv, pos_correct, size, ramp);\n    return mix(uv, rot_uv, filter);\n}\n\nvec2 vortex_pair_warp(vec2 uv,
vec2 pos, vec2 vel)\n{\n    vec2 aspect = vec2(1.,iResolution.y\/iResolution.x);\n    float ramp = 4.;\n\n    float d = 0.125;\n\n    float l = length(vel);\n    vec2 p1
= pos;\n    vec2 p2 = pos;\n\n    if(l > 0.){\n        vec2 normal = normalize(vel.yx * vec2(-1., 1.))\/aspect;\n        p1 = pos - normal * d \/ 2.;\n        p2 = pos +
normal * d \/ 2.;\n    }\n\n    float w = l \/ d * 2.;\n\n    \/\/ two overlapping rotations that would annihilate when they were not displaced.\n    vec2 circle1 =
vortex_warp(uv, p1, d, ramp, vec2(cos(w),sin(w)));\n    vec2 circle2 = vortex_warp(uv, p2, d, ramp, vec2(cos(-w),sin(-w)));\n    return (circle1 + circle2)
\/ 2.;\n}\n\nvoid mainImage( out vec4 fragColor, in vec2 fragCoord )\n{\n    vec2 uv = fragCoord.xy \/ iResolution.xy;\n    vec4 noise = (texture2D(iChannel2,
fragCoord.xy \/ iChannelResolution[2].xy + fract(vec2(42,56)*iGlobalTime))-0.5)*2.;\n\n    if(iFrame<10)\n    {\n        fragColor = noise;\n        return;\n    }\n\n
vec2 mouseV = mouseDelta();\n    vec2 aspect = vec2(1.,iResolution.y\/iResolution.x);\n    vec2 pixelSize = 1. \/ iResolution.xy;\n\n    uv = vortex_pair_warp(uv,
iMouse.xy*pixelSize, mouseV*aspect*1.4);\n    \n    \/\/ expansion\n    vec2 gradientLookupDistance = pixelSize*3.;\n    float expansionFactor = 1.;\n    \n    \/\/
reaction-diffusion  \n    float differentialFactor = 12.\/256.;\n    float increment = - 3.\/256.;\n    float noiseFactor = 2.\/256.;\n    \n    \/\/ rock-paper-scissor\n
float feedBack = 6.\/256.;\n    float feedForward = 6.\/256.;\n\n\tfragColor.r = BlurA(uv + GradientA(uv, gradientLookupDistance, vec4(4.,0.,-2.,0.),
1)*pixelSize*expansionFactor, 0).r;\n\tfragColor.g = BlurA(uv + GradientA(uv, gradientLookupDistance, vec4(0.,4.,0.,-2.), 1)*pixelSize*expansionFactor,
0).g;\n\tfragColor.b = BlurA(uv + GradientA(uv, gradientLookupDistance, vec4(-2.,0.,4.,0.), 1)*pixelSize*expansionFactor, 0).b;\n    fragColor.a = BlurA(uv +
GradientA(uv, gradientLookupDistance, vec4(0.,-2.,0.,4.), 1)*pixelSize*expansionFactor, 0).a;\n\n   \tfragColor += (BlurA(uv, 1) - BlurA(uv, 2))*differentialFactor;\n\n
...
depth = 1; depth < 8; depth++)\n    {\n        if(depth >= level)\n        {\n            break;\n        }\n        uv = lower_right(uv);\n    }\n\n    return
texture2D(iChannel3, uv);\n}\n\nvec2 GradientA(vec2 uv, vec2 d, vec4 selector, int level){\n    vec4 dX = 0.5*BlurA(uv + vec2(1.,0.)*d, level) - 0.5*BlurA(uv -
vec2(1.,0.)*d, level);\n    vec4 dY = 0.5*BlurA(uv + vec2(0.,1.)*d, level) - 0.5*BlurA(uv - vec2(0.,1.)*d, level);\n    return vec2( dot(dX, selector), dot(dY, selector)
);\n}\n\nvec2 rot90(vec2 vector){\n    return vector.yx*vec2(1,-1);\n}\n\nvec2 complex_mul(vec2 factorA, vec2 factorB){\n    return vec2( factorA.x*factorB.x -
factorA.y*factorB.y, factorA.x*factorB.y + factorA.y*factorB.x);\n}\n\nvec2 spiralzoom(vec2 domain, vec2 center, float n, float spiral_factor, float zoopufactor, vec2
...
normal * d \/ 2.;\n    }\n\n    float w = l \/ d * 2.;\n\n    \/\/ two overlapping rotations that would annihilate when they were not displaced.\n    vec2 circle1 =
vortex_warp(uv, p1, d, ramp, vec2(cos(w),sin(w)));\n    vec2 circle2 = vortex_warp(uv, p2, d, ramp, vec2(cos(-w),sin(-w)));\n    return (circle1 + circle2)
\/ 2.;\n}\n\nvoid mainImage( out vec4 fragColor, in vec2 fragCoord )\n{\n    vec2 uv = fragCoord.xy \/ iResolution.xy;\n    vec4 noise = texture2D(iChannel2, fragCoord.xy
\/ iChannelResolution[2].xy + fract(vec2(42,56)*iGlobalTime));\n\n    if(iFrame<10)\n    {\n        fragColor = noise;\n        return;\n    }\n\n\n    uv = 0.5 + (uv -
0.5)*0.99;\n    vec2 pixelSize = 1.\/iResolution.xy;\n    vec2 mouseV = mouseDelta();\n    vec2 aspect = vec2(1.,iResolution.y\/iResolution.x);\n    uv =
vortex_pair_warp(uv, iMouse.xy*pixelSize, mouseV*aspect*1.4);\n\n    float time = float(iFrame)\/60.;\n    uv = uv + vec2(sin(time*0.1 + uv.x*2. +1.) - sin(time*0.214 +
uv.y*2. +1.), sin(time*0.168 + uv.x*2. +1.) - sin(time*0.115 +uv.y*2. +1.))*pixelSize*1.5;\n\n    fragColor = BlurB(uv, 0);\n    fragColor += ((BlurB(uv, 1) - BlurB(uv,
2))*0.5 + (noise-0.5) * 0.004); \n\n    fragColor = clamp(fragColor, 0., 1.);\n\n    \/\/fragColor = noise; \/\/ reset\n}", "name":"Buf B", "description":"",
            "type":"buffer"
        },
        {
            "inputs":

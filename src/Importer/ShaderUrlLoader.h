...
enum UrlLoadingStatus
{
	URL_LOADING_STATUE_OK = 0,
	URL_LOADING_STATUE_IN_PROGRESS,
	URL_LOADING_STATUE_ERROR,
	URL_LOADING_STATUE_Count
};
...
public:
	...
	std::list<ShaderInfos> GetShaderFromUrl(
		std::string vUrl,
		std::string vApiKey,
	...
	std::string GetFileContentFromUrl(
		std::string vUrl,
		std::string vProxyPath,
		std::string vProxyUserPwd,
		int vTimeOutInSecond,
...
private:
	...
	std::list<ShaderInfos> ParseBuffer(
		...
		std::atomic<float>& vGenerationTime,
		std::atomic<bool>& vImportInOneFile);
	void AddUniformsFromCode(ShaderInfos *vShaderInfos, std::string *vCode, ShaderPlaform vShaderPlaform, std::atomic<bool>& vImportInOneFile, std::string *vCommonCode = nullptr);

...
void ShadertoyBackupFileImportDlg::SetFunction_For_CreateManyFilesShader(
	std::function<std::string(std::string, std::list<ShaderInfos>)> vCreateManyFilesShaderFunc)
{
	m_CreateManyFilesShaderFunc = vCreateManyFilesShaderFunc;
}

void ShadertoyBackupFileImportDlg::SetFunction_For_CreateOneFileShader(
	std::function<std::string(std::string, std::list<ShaderInfos>)> vCreateOneFileShaderFunc)
{
	...
	for (const auto& shader : vJsonArray)
	{
		...
		if (shader.contains("info"))
		{
			...
			if (date != "0")
			{
				...
				sha_info.date = ct::toStr("%i/%i/%i %i:%i",
...
void ShadertoyBackupFileImportDlg::prApplyFiltering(const std::string& vSearchPattern)
{
	...
	{
		if (shader.description_for_search.find(pattern) != std::string::npos
			|| shader.tags_for_search.find(pattern) != std::string::npos

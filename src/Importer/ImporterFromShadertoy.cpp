...
std::list<ShaderInfos> ImporterFromShadertoy::ParseBuffer(const std::string& vBuffer, const std::string& /*vId*/, const bool& vImportInOneFile) {
    std::list<ShaderInfos> shaders;

    picojson::value json;
    picojson::parse(json, vBuffer);

    ShaderToyFormat::ShadertoyStruct shStruct = ShaderToyFormat::ShadertoyStruct::ParseShader(json);
        ...
        for (std::list<ShaderToyFormat::ShaderToyRenderPass>::iterator it_rp = shStruct.renderpass.begin(); it_rp != shStruct.renderpass.end(); ++it_rp) {
            ...
            for (std::list<ShaderToyFormat::ShaderToyInput>::iterator it_in = pass.inputs.begin(); it_in != pass.inputs.end(); ++it_in) {
                ...
                if (input.type == "texture") {
                    ...
                    std::string picture =
                        input.filepath;  // FileHelper::Instance()->(input.src GetFileFromUrl(input.src, 0, FILE_LOCATION_Enum::FILE_LOCATION_ASSET_TEXTURE_2D, nullptr);
...
void ImporterFromShadertoy::AddUniformsFromCode(ShaderInfos& vOutShaderInfos,
                                                std::string& vCode,
...
void ImporterFromShadertoy::DoChannelInput(const std::string& vBaseName,
    ...
    if (nOccurs > 0) {
        if (puChannelLinks.find(vChanId) != puChannelLinks.end())  // trouve
        {
            ...
            } else if (puTexturesId.find(id) != puTexturesId.end())  // texture
            {
                ...
                vOutShaderInfos.specific_uniforms +=
            ...
            } else if (puTextures3DId.find(id) != puTextures3DId.end())  // texture3D
            {
                ...
                vOutShaderInfos.specific_uniforms += "uniform sampler3D(volume:file=" + pict + ":format=" + format + ":mipmap=" + mipmap + ":wrap=" + wrap +

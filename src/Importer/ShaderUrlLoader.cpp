...
std::list<ShaderInfos> ShaderUrlLoader::GetShaderFromUrl(std::string vUrl,
                                                         std::string vApiKey,
                            ...
                            if (code == CURLE_OK) {
                                ...
                                if (vWorking) {
                                    if (code == CURLE_OK) {
                                        if (!buffer.empty()) {
                                            shaders = ParseBuffer(
                                                buffer, spf, id, std::ref(vProgress), std::ref(vWorking), std::ref(vGenerationTime), std::ref(vImportInOneFile));
...
void ShaderUrlLoader::ReSavePctureFile(std::string vPictureFile) {
    if (vPictureFile.find(".png") != std::string::npos) {
        ...
        if (image != nullptr) {
            /*int resWrite = */ stbi_write_png(vPictureFile.c_str(), w, h, comp, image, w * comp);
...
std::string ShaderUrlLoader::GetFileContentFromUrl(std::string vUrl,
                                                   std::string vProxyPath,
                                                   std::string vProxyUserPwd,
                                                   int vTimeOutInSecond,
                                                   std::atomic<UrlLoadingStatus> &vUrlLoadingStatus,
...
std::list<ShaderInfos> ShaderUrlLoader::ParseBuffer(std::string vBuffer,
                                                    ShaderPlaform vSPF,
                                                    std::string vId,
                                                    std::atomic<float> &vProgress,
                                                    std::atomic<bool> &vWorking,
    ...
    if (vWorking) {
        if (vSPF == ShaderPlaform::SPF_GLSLSANDBOX) {
            ...
            GlslSandboxFormat::GlslSandboxStruct glStruct = GlslSandboxFormat::GlslSandboxStruct::ParseBuffer(vBuffer);
            ...
            VertexShaderArtFormat::VertexShaderArtStruct vsaStruct = VertexShaderArtFormat::VertexShaderArtStruct::ParseBuffer(vBuffer);
            ...
            ShaderToyFormat::ShadertoyStruct shStruct = ShaderToyFormat::ShadertoyStruct::ParseBuffer(vBuffer);
            ...
            if (!shStruct.error.empty()) {
                ...
                error->error +=
                    "If the message say : 'Invalid Key',\n its because,\n ShaderToy Need an Api Key for do import working.\n Go in https://www.shadertoy.com/myapps for "
...
void ShaderUrlLoader::AddUniformsFromCode(ShaderInfos *vShaderInfos,
                                          std::string *vCode,
                                          ShaderPlaform vShaderPlaform,
                                          std::atomic<bool> &vImportInOneFile,
                                          std::string *vCommonCode) {
    if (vShaderInfos) {
    ...
    if (nOccurs > 0) {
        if (puChannelLinks.find(vChanId) != puChannelLinks.end())  // trouve
        {
            ...
            } else if (puTexturesId.find(id) != puTexturesId.end())  // texture
            {
                ...
                vShaderInfos->specific_uniforms +=
                    "uniform sampler2D(picture:choice=" + pict + ":flip=" + flip + ":mipmap=" + mipmap + ":wrap=" + wrap + ":filter=" + filter + ") " + vBaseName + ";\n";
            ...
            } else if (puTextures3DId.find(id) != puTextures3DId.end())  // texture3D
            {
                ...
                vShaderInfos->specific_uniforms += "uniform sampler3D(volume:file=" + pict + ":format=" + format + ":mipmap=" + mipmap + ":wrap=" + wrap +
                    ":filter=" + filter + ") " + vBaseName + ";\n";

...
public:
    std::list<ShaderInfos> ParseBuffer(const std::string& vBuffer, const std::string& vId, const bool& vImportInOneFile = true);

private:
    void AddUniformsFromCode(ShaderInfos& vOutShaderInfos,
                             std::string& vCode,
                             const ShaderPlaform& vShaderPlaform,
                             const bool& vImportInOneFile,
                             const std::string* vCommonCodePtr = nullptr);

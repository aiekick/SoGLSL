...
Shader::Shader(const GuiBackend_Window& vWin, std::string vShaderName) 
	: puWindow(vWin), _isValid(false), puShaderName(vShaderName), puuProgram(0), linked(-1), err(0)
{
...
ProgramMsg Shader::InitAndLinkShaderProgram(
	bool vIsCompute,
	...
	std::string version)
{
	...
	if (!vIsCompute)
	{
		...
		const GLuint vertexShader = LoadFromString(ShaderTypeEnum::SHADER_TYPE_VERTEX,
		                                           shaderCode.GetSection("VERTEX"), version);

		const GLuint geometryShader = LoadFromString(ShaderTypeEnum::SHADER_TYPE_GEOMETRY,
		                                             shaderCode.GetSection("GEOMETRY"), version);

		const GLuint tessControlShader = LoadFromString(ShaderTypeEnum::SHADER_TYPE_TESSELATION_CONTROL,
		                                                shaderCode.GetSection("TESSCONTROL"), version);

		const GLuint tessEvalShader = LoadFromString(ShaderTypeEnum::SHADER_TYPE_TESSELATION_EVAL,
		                                             shaderCode.GetSection("TESSEVAL"), version);

		const GLuint fragmentShader = LoadFromString(ShaderTypeEnum::SHADER_TYPE_FRAGMENT,
		                                             shaderCode.GetSection("FRAGMENT"), version);
	...
	{
		// Load the vertex/fragment shaders
		const GLuint computeShader = LoadFromString(ShaderTypeEnum::SHADER_TYPE_COMPUTE,
		                                            shaderCode.GetSection("COMPUTE"), version);
	...
	std::string /*version*/)
{
	...
	if (type == ShaderTypeEnum::SHADER_TYPE_GEOMETRY && !GLVersionChecker::Instance()->m_GeometryShaderSupported) return 0;
	if (type == ShaderTypeEnum::SHADER_TYPE_TESSELATION_CONTROL && !GLVersionChecker::Instance()->m_TesselationShaderSupported) return 0;
	if (type == ShaderTypeEnum::SHADER_TYPE_TESSELATION_EVAL && !GLVersionChecker::Instance()->m_TesselationShaderSupported) return 0;
	if (type == ShaderTypeEnum::SHADER_TYPE_COMPUTE && !GLVersionChecker::Instance()->m_ComputeShaderSupported) return 0;
	...
	if (!shaderSrc.empty())
	{
		...
		if (shader != 0)
		{
			...
			std::string typeStr;
			if (type == ShaderTypeEnum::SHADER_TYPE_FRAGMENT) typeStr = "FRAGMENT";
			if (type == ShaderTypeEnum::SHADER_TYPE_VERTEX) typeStr = "VERTEX";
			if (type == ShaderTypeEnum::SHADER_TYPE_GEOMETRY) typeStr = "GEOMETRY";
			if (type == ShaderTypeEnum::SHADER_TYPE_TESSELATION_CONTROL) typeStr = "TESSCONTROL";
			if (type == ShaderTypeEnum::SHADER_TYPE_TESSELATION_EVAL) typeStr = "TESSEVAL";
			if (type == ShaderTypeEnum::SHADER_TYPE_COMPUTE) typeStr = "COMPUTE";
	...
	else
	{
		if (type != ShaderTypeEnum::SHADER_TYPE_GEOMETRY && 
			type != ShaderTypeEnum::SHADER_TYPE_TESSELATION_CONTROL && 

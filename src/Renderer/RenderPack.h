...
public:
    ...
    static RenderPackPtr createComputeWithFileWithoutLoading(const GuiBackend_Window& vWin, const std::string& vName, ct::ivec3 vSize, ShaderKeyPtr vShaderKey);
    static RenderPackPtr createComputeWithFile(const GuiBackend_Window& vWin, const std::string& vName, ct::ivec3 vSize, ShaderKeyPtr vShaderKey);

    static RenderPackPtr createBufferWithFileWithoutLoading(const GuiBackend_Window& vWin,
                                                            const std::string& vName,
                                                            ct::ivec3 vSize,
                                                            ShaderKeyPtr vShaderKey,
                                                            bool vUseZBuffer,
                                                            bool vUseFBO,
                                                            bool vUseRenderBuffer = false,
                                                            bool vUseFloatBuffer = false);
    static RenderPackPtr createBufferWithFile(const GuiBackend_Window& vWin,
                                              const std::string& vName,
                                              ct::ivec3 vSize,
                                              ShaderKeyPtr vShaderKey,
                                              bool vUseZBuffer,
                                              bool vUseFBO,
                                              bool vUseRenderBuffer = false,
                                              TextureParamsStruct* vTexParam = nullptr,
                                              bool vUseFloatBuffer = false);
...
public:
    ...
    SyntaxErrors* GetSyntaxErrors() {
        if (puShaderKey)
            return &puShaderKey->puSyntaxErrors;
        return nullptr;
    }
    void SetCanWeRender(bool vCanWeRender) {
        puCanWeRender = vCanWeRender;
    }
    bool GetCanWeRender() {
        return puCanWeRender;
    }
    int GetFBOId() {
        if (puFrameBuffer != nullptr)
            return puFrameBuffer->getBackFboID();
        return -1;
    }
    FrameBuffersPipeLinePtr GetPipe() {
        return puFrameBuffer;
    }
    BaseModelWeak GetModel() {
        return puModel_Render;
    }
    std::shared_ptr<RecordBuffer> GetRecordBuffer() {
        return puRecordBuffer;
    }

    ct::ivec3 GetSize() {
        return puMaxScreenSize;
    }
    int GetTextureId(int i = 0) {
        if (puFrameBuffer != nullptr)
            return puFrameBuffer->getBackTextureID(i);
        return -1;
    }
    ctTexturePtr GetTexture(int i = 0) {
        if (puFrameBuffer != nullptr)
            return puFrameBuffer->getBackTexture(i);
        return nullptr;
    }
    int GetRenderBufferId(int i = 0) {
        if (puFrameBuffer != nullptr)
            return puFrameBuffer->getBackRenderBufferID(i);
        return -1;
    }
    bool IsUseZBuffer() {
        return puShaderKey->puShaderGlobalSettings.useZBuffer;
    }
    ShaderPtr GetShader() {
        return puShader;
    }
    bool IsNewCompilationNeeded() {
        return puNewCompilationNeeded;
    }
    void NeedNewCompilation() {
        puNewCompilationNeeded = true;
    }
    RenderPack_Type GetRenderPackType() {
        return puRenderPackType;
    }

    GuiBackend_Window GetGuiBackendWindow() {
        return puWindow;
    }
...
public:
    ...
    // GPU
    ////////////////////////////////////////////////
    bool BindFBO(FrameBuffersPipeLinePtr vPipe = nullptr, const bool& vDontUseAnyFBO = false);
    ...
    bool ComputeShader();
    void UnBindFBO(const bool& vUpdateMipMap = true, const bool& vSwitchBuffers = true, FrameBuffersPipeLinePtr vPipe = nullptr, const bool& vDontUseAnyFBO = false);
    void UpdateMipMap();
    ...
    // CPU
    ////////////////////////////////////////////////
    bool RenderNode(GLenum* vRenderMode = nullptr,
    ...
    void SetComputeDefaultParams(const std::string& vName, ct::ivec3 vSize, ShaderKeyPtr vShaderKey);
    void SetBufferDefaultParams(const std::string& vName,
                                ct::ivec3 vSize,
                                ShaderKeyPtr vShaderKey,
                                bool vUseZBuffer,
                                bool vUseFBO,
    ...
    void UpdateUniforms(UpdateUniformFuncSignature vSpecificUniformsFunc,
    ...
    void UpdateUniform(UniformVariantPtr vUniPtr,
                       DisplayQualityType vDisplayQuality,
                       MouseInterface* vMouse,
                       CameraInterface* vCamera,
                       MeshRectType* vMouseRect = nullptr);
    void UpdateMouseWidgets(UniformVariantPtr vUniPtr,
                            DisplayQualityType vDisplayQuality,
                            MouseInterface* vMouse,
                            CameraInterface* vCamera,
                            MeshRectType* vMouseRect = nullptr);
    ...
    void ConfigureBufferParams(std::string vFormat = "float",
                               bool vMipMap = false,
                               int vMaxMipMaplvl = 1000,
                               std::string vWrap = "clamp",
                               std::string vFilter = "linear",
                               bool vReloadFBO = true);
...
private:
    ...
    bool InitComputeWithFile(const GuiBackend_Window& vWin, const std::string& vName, ct::ivec3 vSize, ShaderKeyPtr vShaderKey);
    bool InitBufferWithFile(const GuiBackend_Window& vWin,
                            const std::string& vName,
                            ct::ivec3 vSize,
                            ShaderKeyPtr vShaderKey,
                            bool vUseZBuffer,
